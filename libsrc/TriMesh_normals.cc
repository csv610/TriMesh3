...
static void normals_from_tstrips_Max(vector<int>& tstrips, vector<point>& vertices, vector<vec>& normals) {
	if (tstrips.back() < 0) {
		bool flip = false;
		for (size_t i = 2; i < tstrips.size(); i++) {
			...
			vec facenormal = flip ? cross(b, a) : cross(a, b);
...
static void normals_from_tstrips_area(vector<int>& tstrips, vector<point>& vertices, vector<vec>& normals) {
	if (tstrips.back() < 0) {
		bool flip = false;
		for (size_t i = 2; i < tstrips.size(); i++) {
			if (tstrips[i] < 0) {
...
static void normals_from_faces_Max(vector<TriMesh::Face>& faces, vector<point>& vertices, vector<vec>& normals) {
	int nf = faces.size();
...
static void normals_from_faces_area(vector<TriMesh::Face>& faces, vector<point>& vertices, vector<vec>& normals) {
	int nf = faces.size();
#pragma omp parallel for
	for (int i = 0; i < nf; i++) {
		const point& p0 = vertices[faces[i][0]];
...
#pragma omp parallel for
	for (int i = 0; i < nv; i++) {
		...
		for (int j = 0; j < actual_k; j++) {
			...
			for (int l = 0; l < 3; l++)
				for (int m = 0; m < 3; m++) C[l][m] += d[l] * d[m];
		}
		float e[3];
		eigdc<float, 3>(C, e);
		normals[i].set(C[0][0], C[1][0], C[2][0]);
		if (dot(normals[i], ref) < 0.0f) normals[i] = -normals[i];
	}
}
...
void TriMesh::need_normals(bool simple_area_weighted /* = false */) {
	...
	int nv = vertices.size();
	if (!nv || int(normals.size()) == nv) return;
...
#pragma omp parallel for
	for (int i = 0; i < nv; i++) normalize(normals[i]);
