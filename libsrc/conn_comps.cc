...
#define FOR_EACH_ADJACENT_FACE(mesh, v, f)                    \
	for (size_t f_ind = 0, f = mesh->adjacentfaces[v][0]; \
	     (f_ind < mesh->adjacentfaces[v].size()) && ((f = mesh->adjacentfaces[v][f_ind]) || 1); f_ind++)
...
class CompareArrayElements {
	...
	bool operator()(int i1, int i2) const { return (a[i1] > a[i2]); }
};
...
static bool connected(const TriMesh* mesh, int f1, int f2, bool conn_vert) {
	...
	if (conn_vert)
		return f10 == f20 || f10 == f21 || f10 == f22 || f11 == f20 || f11 == f21 || f11 == f22 || f12 == f20 ||
		       f12 == f21 || f12 == f22;
	else
		return (f10 == f20 && (f11 == f22 || f12 == f21)) || (f10 == f21 && (f11 == f20 || f12 == f22)) ||
		       (f10 == f22 && (f11 == f21 || f12 == f20)) || (f11 == f20 && f12 == f22) ||
		       (f11 == f21 && f12 == f20) || (f11 == f22 && f12 == f21);
}

// Helper function for find_comps, below.  Finds and marks all the faces
// connected to f.
static void find_connected(const TriMesh* mesh, vector<int>& comps, vector<int>& compsizes, int f, int whichcomponent,
			   bool conn_vert) {
	stack<int> s;
	s.push(f);
	while (!s.empty()) {
		...
		for (int i = 0; i < 3; i++) {
			...
			FOR_EACH_ADJACENT_FACE(mesh, vert, adjface) {
				if (comps[adjface] != NO_COMP || !connected(mesh, adjface, currface, conn_vert))
					continue;
...
static void sort_comps(vector<int>& comps, vector<int>& compsizes) {
	...
	for (size_t i = 0; i < comp_pointers.size(); i++) comp_pointers[i] = i;

	sort(comp_pointers.begin(), comp_pointers.end(), CompareArrayElements<vector<int> >(compsizes));

	vector<int> remap_table(comp_pointers.size());
	for (size_t i = 0; i < comp_pointers.size(); i++) remap_table[comp_pointers[i]] = i;
	for (size_t i = 0; i < comps.size(); i++) comps[i] = remap_table[comps[i]];

	vector<int> newcompsizes(compsizes.size());
	for (size_t i = 0; i < compsizes.size(); i++) newcompsizes[i] = compsizes[comp_pointers[i]];
...
void find_comps(TriMesh* mesh, vector<int>& comps, vector<int>& compsizes, bool conn_vert /* = false */) {
	...
	if (compsizes.size() > 1) sort_comps(comps, compsizes);
}
...
void select_comp(TriMesh* mesh, const vector<int>& comps, int whichcc) {
	...
	for (int i = 0; i < numfaces; i++) {
		if (comps[i] != whichcc) toremove[i] = true;
	}
...
void select_big_comps(TriMesh* mesh, const vector<int>& comps, const vector<int>& compsizes, int min_size,
		      int total_largest /* = std::numeric_limits<int>::max() */) {
	int ncomp = compsizes.size();
	int keep_last = min(ncomp - 1, total_largest - 1);
	while (keep_last > -1 && compsizes[keep_last] < min_size) keep_last--;

	int numfaces = mesh->faces.size();
	vector<bool> toremove(numfaces, false);
	for (int i = 0; i < numfaces; i++) {
		if (comps[i] > keep_last) toremove[i] = true;
	}
...
void select_small_comps(TriMesh* mesh, const vector<int>& comps, const vector<int>& compsizes, int max_size,
			int total_smallest /* = std::numeric_limits<int>::max() */) {
	int ncomp = compsizes.size();
	int keep_first = max(0, ncomp - total_smallest);
	while (keep_first < ncomp && compsizes[keep_first] > max_size) keep_first++;

	int numfaces = mesh->faces.size();
	vector<bool> toremove(numfaces, false);
	for (int i = 0; i < numfaces; i++) {
		if (comps[i] < keep_first) toremove[i] = true;
	}
