...
typedef void(APIENTRYP glShaderSource_t)(GLuint shader, GLsizei count, const GLchar* const* string,
					 const GLint* length);
...
typedef void(APIENTRYP glGetActiveUniform_t)(GLuint program, GLuint index, GLsizei bufSize, GLsizei* length,
...
typedef void(APIENTRYP glVertexAttribPointer_t)(GLuint index, GLint size, GLenum type, GLboolean normalized,
...
typedef void(APIENTRYP glGetActiveAttrib_t)(GLuint program, GLuint index, GLsizei bufSize, GLsizei* length, GLint* size,
...
typedef void(APIENTRYP glFramebufferTexture2D_t)(GLenum target, GLenum attachment, GLenum textarget, GLuint texture,
						 GLint level);
...
bool GLManager::glGetProcs() {
	...
	if (already_tried) return true;
	already_tried = true;

#define GETPROC(name, is_required)                                                   \
	do {                                                                         \
...
class GLManager::ShaderDefine : public pair<string, string> {
    public:
	ShaderDefine(const string& defname, const string& value) : pair<string, string>(defname, value) {}
};
...
struct GLManager::AttributeInfo {
	...
	GLint location;
	AttributeInfo() : buf(0), p(0), floats_per_attribute(0), stride(0), offset(0), location(-1) {}
};
...
class GLManager::ShaderInfo {
	...
	ShaderInfo(const string& name_, unsigned vshader_, unsigned fshader_, unsigned program_, int nuniforms,
		   int nattributes)
	    : name(name_),
	      vshader(vshader_),
	      fshader(fshader_),
...
GLManager::GLManager()
    : current_shader(0),
      ...
      have_restart_fixed(false),
      have_restart(false),
      have_restart_nv(false),
      checked_clamp(false),
      checked_vao(false),
      vao(0),
      eprintf_hook(nullptr) {
...
unsigned GLManager::make_shader(const char* name, const char* vert_text, const char* frag_text) {
	// Sanity checking
	if (!name || !*name) {
	...
	if (!glGetProcs()) return 0;

	// Build strings containing defines plus shader code
	string defines_string;
	for (size_t i = 0; i < defines.size(); i++) {
		defines_string += "#define " + defines[i]->first + " " + defines[i]->second + "\n";
	}
	...
	shaders.push_back(new ShaderInfo(name, vshader, fshader, program, nuniforms, nattributes));
	...
	for (int i = 0; i < nuniforms; i++) {
		GLint size;
		GLenum type;
		glGetActiveUniform(program, i, len, nullptr, &size, &type, &uname[0]);
		UniformInfo& ui = shaders.back()->uniform_info[i];
		ui.name = string(&uname[0]);
		ui.type = type;
		if (type == GL_SAMPLER_2D) shaders.back()->texunit_binding.push_back(0);
	...
	for (int i = 0; i < nattributes; i++) {
		GLint size;
		GLenum type;
		glGetActiveAttrib(program, i, len, nullptr, &size, &type, &aname[0]);
...
unsigned GLManager::make_shader_from_files(const char* name, const char* vert_filename, const char* frag_filename) {
...
bool GLManager::use_shader(unsigned ind) {
	if (current_shader->program == ind) return true;

	// Disable old attribute arrays
	if (current_shader->have_vertex) glDisableClientState(GL_VERTEX_ARRAY);
	if (current_shader->have_normal) glDisableClientState(GL_NORMAL_ARRAY);
	if (current_shader->have_color3 || current_shader->have_color4) glDisableClientState(GL_COLOR_ARRAY);
	if (current_shader->have_texcoord) glDisableClientState(GL_TEXTURE_COORD_ARRAY);
	...
	for (size_t i = 0; i < current_shader->attribute_info.size(); i++) {
		if (!current_shader->have_attribute[i]) continue;
...
void GLManager::clear_shader(unsigned ind) {
	...
	if ((*it)->program) glDeleteProgram((*it)->program);
	if ((*it)->fshader) glDeleteShader((*it)->fshader);
	if ((*it)->vshader) glDeleteShader((*it)->vshader);
	delete (*it);
	shaders.erase(it);
}
...
void GLManager::clear_shaders() {
	use_shader();
	while (shaders.size() > 1) clear_shader(shaders.back()->program);
}
...
bool GLManager::uniform_boilerplate(int ind, unsigned type) {
	...
	for (which = 0; which < current_shader->uniform_info.size(); which++)
		if (current_shader->uniform_info[which].location == ind) break;
...
void GLManager::uniformTexture(int ind, unsigned texind) {
	if (!uniform_boilerplate(ind, GL_SAMPLER_2D)) return;
	unsigned texunit = 0;
	for (int i = 0; i < ind; i++) {
		if (current_shader->uniform_info[i].type == GL_SAMPLER_2D) texunit++;
	}
...
void GLManager::clear_uniforms() {
	for (size_t i = 0; i < current_shader->have_uniform.size(); i++) current_shader->have_uniform[i] = false;
	current_shader->checked = false;
}

// Set an attribute to a (client-side) array or vector
void GLManager::attributearray(int ind, int floats_per_attribute, const float* p, size_t stride, size_t offset) {
	if (!current_shader->program) {
	...
	for (which = 0; which < current_shader->attribute_info.size(); which++)
		if (current_shader->attribute_info[which].location == ind) break;
	if (which == current_shader->attribute_info.size()) {
		eprintf("GLManager::attribute : invalid attribute\n");
		return;
	}
	if (current_shader->attribute_info[which].floats_per_attribute != floats_per_attribute) {
	...
	ai.stride = stride;
	ai.offset = offset;
	glVertexAttribPointer(ind, floats_per_attribute, GL_FLOAT, GL_FALSE, stride, p + offset / sizeof(float));
	glEnableVertexAttribArray(ind);
}

// Set an attribute to a buffer
void GLManager::attributearray(int ind, int floats_per_attribute, unsigned buf, size_t stride, size_t offset) {
	if (!current_shader->program) {
		eprintf("GLManager::attribute : no active shader program\n");
		return;
	}
	...
	if (!use_buffer(buf)) return;
	current_shader->have_attribute[which] = true;
	ai.buf = buf;
	ai.p = 0;
	ai.stride = stride;
	ai.offset = offset;
	glVertexAttribPointer(ind, floats_per_attribute, GL_FLOAT, GL_FALSE, stride, GL_VBO_OFFSET(offset));
	glEnableVertexAttribArray(ind);
}
...
bool GLManager::use_buffer(unsigned buf) {
	if (current_buffer->ind == buf) return true;
	if (!glGetProcs()) return false;
...
void GLManager::clear_buffer(unsigned buf) {
	if (current_buffer->ind == buf) use_buffer();
	...
	if ((*it)->ind) glDeleteBuffers(1, &((*it)->ind));
	delete (*it);
	buffers.erase(it);
}
...
void GLManager::clear_buffers() {
	use_buffer();
	while (buffers.size() > 1) clear_buffer(buffers.back()->ind);
}
...
bool GLManager::use_ibuffer(unsigned buf) {
	if (current_ibuffer->ind == buf) return true;
	if (!glGetProcs()) return false;
...
void GLManager::clear_ibuffer(unsigned buf) {
	if (current_ibuffer->ind == buf) use_ibuffer();
...
void GLManager::clear_ibuffers() {
	use_ibuffer();
	while (ibuffers.size() > 1) clear_ibuffer(ibuffers.back()->ind);
}
...
// Textures
unsigned GLManager::make_texture(int w, int h, const void* p, unsigned format, unsigned type, unsigned internalformat) {
	if (!glGetProcs()) return 0;
	unsigned texind;
...
void GLManager::update_texture(unsigned texind, int w, int h, const void* p, unsigned format, unsigned type,
			       unsigned internalformat) {
	if (!use_texture(texind)) return;
	if (!internalformat) internalformat = format;
	...
	if (flipping_textures) {
		...
		for (int i = 0; i < h; i++) {
			memcpy(buf + rowlen * (h - 1 - i), (const unsigned char*) p + rowlen * i, rowlen);
		}
	...
	if (internalformat == GL_R16F || internalformat == GL_R32F || internalformat == GL_RG16F ||
	    internalformat == GL_RG32F || internalformat == GL_RGB16F || internalformat == GL_RGB32F ||
...
bool GLManager::use_texture(unsigned texind) {
	...
	if (current_texture->ind == texind && texunit < current_shader->texunit_binding.size() &&
	...
	glBindTexture(GL_TEXTURE_2D, texind);
	if (texunit < current_shader->texunit_binding.size()) current_shader->texunit_binding[texunit] = texind;
...
void GLManager::clear_texture(unsigned texind) {
	...
	if ((*it)->ind) glDeleteTextures(1, &((*it)->ind));
	delete (*it);
	textures.erase(it);
}
...
void GLManager::clear_textures() {
	use_texture();
	while (textures.size() > 1) clear_texture(textures.back()->ind);
}

// Render to texture
void GLManager::rendertarget(unsigned texind, unsigned depthbufferind) {
	if (!glGetProcs() || glversion() < 3.0f) return;
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texind, 0);
	if (depthbufferind)
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depthbufferind, 0);
...
bool GLManager::shader_ready() {
	...
	if (!program || current_shader->checked) return true;
	...
	for (size_t i = 0; i < current_shader->have_uniform.size(); i++) {
		if (current_shader->uniform_info[i].location < 0 || current_shader->have_uniform[i]) continue;
	...
	// Check attributes
	for (size_t i = 0; i < current_shader->have_attribute.size(); i++) {
		if (current_shader->attribute_info[i].location < 0 || current_shader->have_attribute[i]) continue;
		...
		eprintf("GLManager::shader_ready : shader %s failed to set attribute %s\n",
			current_shader->name.c_str(), current_shader->attribute_info[i].name.c_str());
		ready = false;
	}
...
void GLManager::draw_tris(std::size_t start, std::size_t ntris) {
	shader_ready();
	glDrawElements(GL_TRIANGLES, 3 * ntris, GL_UNSIGNED_INT, GL_VBO_OFFSET(start * sizeof(unsigned)));
}

void GLManager::draw_tstrips(const unsigned* p, std::size_t start, std::size_t nstripinds) {
	shader_ready();
	if (enable_restart()) {
		glDrawElements(GL_TRIANGLE_STRIP, nstripinds, GL_UNSIGNED_INT, p + start);
	} else {
		p += start;
		...
		while (p < end) {
			...
			glDrawElements(GL_TRIANGLE_STRIP, p - stripstart, GL_UNSIGNED_INT, stripstart);
			p++;
		}
	}
}
...
void GLManager::draw_tstrips(std::size_t start, std::size_t nstripinds) {
	shader_ready();
	if (enable_restart()) {
		glDrawElements(GL_TRIANGLE_STRIP, nstripinds, GL_UNSIGNED_INT, GL_VBO_OFFSET(start * sizeof(unsigned)));
	} else {
		unsigned buf = 0;
		const unsigned* p = 0;
...
unsigned GLManager::shader(const char* name, bool print_if_not_found) {
	for (size_t i = 1; i < shaders.size(); i++) {
		if (strcmp(name, shaders[i]->name.c_str()) == 0) return shaders[i]->program;
	}
	if (print_if_not_found) eprintf("GLManager::shader : lookup of %s failed\n", name);
	return 0;
}
...
int GLManager::uniform(const char* name, bool print_if_not_found) {
	...
	if (ret < 0 && print_if_not_found) eprintf("GLManager::uniform : lookup of %s failed\n", name);
	return ret;
}
...
int GLManager::attribute(const char* name, bool print_if_not_found) {
	...
	if (ret < 0 && print_if_not_found) eprintf("GLManager::attribute : lookup of %s failed\n", name);
	return ret;
}

unsigned GLManager::buffer(const void* p, bool print_if_not_found) {
	for (size_t i = 1; i < buffers.size(); i++) {
		if (buffers[i]->p == p) return buffers[i]->ind;
	}
	if (print_if_not_found) eprintf("GLManager::buffer : lookup of %p failed\n", p);
	return 0;
}

unsigned GLManager::ibuffer(const void* p, bool print_if_not_found) {
	for (size_t i = 1; i < ibuffers.size(); i++) {
		if (ibuffers[i]->p == p) return ibuffers[i]->ind;
	}
	if (print_if_not_found) eprintf("GLManager::ibuffer : lookup of %p failed\n", p);
	return 0;
}

unsigned GLManager::texture(const void* p, bool print_if_not_found) {
	for (size_t i = 1; i < textures.size(); i++) {
		if (textures[i]->p == p) return textures[i]->ind;
	}
	if (print_if_not_found) eprintf("GLManager::texture : lookup of %p failed\n", p);
	return 0;
}
...
bool GLManager::extension_supported(const char* name) {
	...
	for (;;) {
		ext = strstr(ext, name);
		if (!ext || ext + namelen > ext_end) break;
		if (*(ext + namelen) == ' ' || *(ext + namelen) == '\0') return true;
		ext++;
	}
	return false;
}
...
bool GLManager::slow_tstrips() {
	...
	return strstr((const char*) glGetString(GL_RENDERER), "Intel") && !have_primitive_restart();
}

// Check whether we have primitive restart in some form
void GLManager::check_restart() {
	if (checked_restart) return;
	...
	have_restart_nv =
	    (extension_supported("GL_NV_primitive_restart") && glGetProcs() && glPrimitiveRestartIndexNV != nullptr);
}
...
void GLManager::check_clamp() {
	...
	} else if (extension_supported("GL_ARB_color_buffer_float") && glGetProcs() && glClampColorARB != nullptr) {
...
bool GLManager::shader_ok(unsigned shader_num, const char* what, const char* name) {
	...
	glGetShaderInfoLog(shader_num, log_length, nullptr, &log[0]);
...
bool GLManager::program_ok(unsigned prog_num, const char* what, const char* name) {
	...
	if (log_length > 5) {
		vector<char> log(log_length);
		glGetProgramInfoLog(prog_num, log_length, nullptr, &log[0]);
