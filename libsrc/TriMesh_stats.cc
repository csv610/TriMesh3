...
float TriMesh::stat(StatOp op, StatVal val) {
	...
	switch (val) {
		case STAT_VALENCE: {
			...
			for (int i = 0; i < nv; i++) vals.push_back((float) neighbors[i].size());
			break;
		}
		case STAT_FACEAREA: {
			need_faces();
			int nf = faces.size();
			for (int i = 0; i < nf; i++) vals.push_back(len(trinorm(i)));
			break;
		}
		case STAT_ANGLE: {
			need_faces();
			int nf = faces.size();
			for (int i = 0; i < nf; i++)
				for (int j = 0; j < 3; j++) vals.push_back(cornerangle(i, j));
			break;
		}
		case STAT_DIHEDRAL: {
			need_across_edge();
		...
		case STAT_EDGELEN: {
			...
			for (int i = 0; i < nf; i++)
				for (int j = 0; j < 3; j++)
					vals.push_back(dist(vertices[faces[i][j]], vertices[faces[i][NEXT_MOD3(j)]]));
			break;
		}
		case STAT_X: {
			int nv = vertices.size();
			for (int i = 0; i < nv; i++) vals.push_back(vertices[i][0]);
			break;
		}
		case STAT_Y: {
			int nv = vertices.size();
			for (int i = 0; i < nv; i++) vals.push_back(vertices[i][1]);
			break;
		}
		case STAT_Z: {
			int nv = vertices.size();
			for (int i = 0; i < nv; i++) vals.push_back(vertices[i][2]);
			break;
		}
		default:
			return 0.0f;
	...
	switch (op) {
		...
		case STAT_RMS:
			return sqrt(accumulate(vals.begin(), vals.end(), 0.0f) / n);
		...
		case STAT_STDEV: {
			float mean = accumulate(vals.begin(), vals.end(), 0.0f) / n;
			for (int i = 0; i < n; i++) vals[i] = sqr(vals[i] - mean);
			return sqrt(accumulate(vals.begin(), vals.end(), 0.0f) / n);
		}
