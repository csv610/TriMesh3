...
#define GET_WORD()                                                \
	do {                                                      \
		if (fscanf(f, " %1023s", buf) != 1) return false; \
	} while (0)
#define COND_READ(cond, where, len)                                        \
	do {                                                               \
		if ((cond) && !fread(&(where), (len), 1, f)) return false; \
	} while (0)
#define FPRINTF(...)                                        \
	do {                                                \
		if (fprintf(__VA_ARGS__) < 0) return false; \
	} while (0)
#define FWRITE(ptr, size, nmemb, stream)                                               \
	do {                                                                           \
		if (fwrite((ptr), (size), (nmemb), (stream)) != (nmemb)) return false; \
	} while (0)
#define LINE_IS(text) begins_with(buf, text)

constexpr float BIGNUM = 1.0e10f;
...
static bool read_verts_bin(FILE* f, TriMesh* mesh, bool& need_swap, int nverts, int vert_len, int vert_pos,
			   int vert_norm, int vert_color, bool float_color, int vert_conf);
static bool slurp_verts_bin(FILE* f, TriMesh* mesh, bool need_swap, int nverts);
static bool read_verts_asc(FILE* f, TriMesh* mesh, int nverts, int vert_len, int vert_pos, int vert_norm,
			   int vert_color, bool float_color, int vert_conf);
static bool read_faces_bin(FILE* f, TriMesh* mesh, bool need_swap, int nfaces, int face_len, int face_count,
			   int face_idx);
static bool read_faces_asc(FILE* f, TriMesh* mesh, int nfaces, int face_len, int face_count, int face_idx,
...
static bool write_ply_ascii(TriMesh* mesh, FILE* f, bool write_norm, bool write_grid, bool float_color);
static bool write_ply_binary(TriMesh* mesh, FILE* f, bool little_endian, bool write_norm, bool write_grid,
			     bool float_color);
...
static bool write_cc(TriMesh* mesh, FILE* f, const char* filename, bool write_norm, bool float_color);
static bool write_dae(TriMesh* mesh, FILE* f);
static bool write_verts_asc(TriMesh* mesh, FILE* f, const char* before_vert, const char* before_norm,
			    const char* before_color, bool float_color, const char* before_conf,
			    const char* after_line);
static bool write_verts_bin(TriMesh* mesh, FILE* f, bool need_swap, bool write_norm, bool write_color, bool float_color,
			    bool write_conf);
static bool write_faces_asc(TriMesh* mesh, FILE* f, const char* before_face, const char* after_line);
static bool write_faces_bin(TriMesh* mesh, FILE* f, bool need_swap, int before_face_len, const char* before_face,
...
static void pushback(const char* buf, FILE* f) {
	const char* c = buf;
	while (*c) c++;
	while ((--c) >= buf) ungetc(*c, f);
}

// std::string versions of read/write
::std::unique_ptr<TriMesh> TriMesh::read(const ::std::string& filename) {
...
// subclassing easier.
::std::unique_ptr<TriMesh> TriMesh::read(const char* filename) {
	auto mesh = ::std::make_unique<TriMesh>();

	if (read_helper(filename, mesh.get())) return mesh;
...
bool TriMesh::read_helper(const char* filename, TriMesh* mesh) {
	...
	if (c == 'p') {
		...
		if (strncmp(buf, "ly", 2) == 0) ok = read_ply(f, mesh);
	} else if (c == 0x4d) {
		int c2 = fgetc(f);
		ungetc(c2, f);
		ungetc(c, f);
		if (c2 == 0x4d) ok = read_3ds(f, mesh);
	} else if (c == 'V') {
		char buf[5];
		if (!fgets(buf, 5, f)) {
			eprintf("Can't read header.\n");
		...
		if (strncmp(buf, "IVID", 4) == 0) ok = read_vvd(f, mesh);
	} else if (c == '#') {
		char buf[1024];
		GET_LINE();
		if (LINE_IS("material") || LINE_IS("vertex") || LINE_IS("shape_")) {
			// Assume a ray file
			pushback(buf, f);
			ungetc(c, f);
			ok = read_ray(f, mesh);
	...
	} else if (c == 'v' || c == 'u' || c == 'f' || c == 'g' || c == 's' || c == 'o' || c == 'm') {
		// Assume an obj file
		ungetc(c, f);
		ok = read_obj(f, mesh);
	} else if (c == 'O') {
		...
		if (strncmp(buf, "FF", 2) == 0) ok = read_off(f, mesh);
	} else if (isdigit(c)) {
...
static bool read_ply(FILE* f, TriMesh* mesh) {
	...
	// Read file format
	GET_LINE();
	while (buf[0] && isspace(buf[0])) GET_LINE();
	...
	while (LINE_IS("obj_info") || LINE_IS("comment")) {
		if (LINE_IS("obj_info num_cols")) sscanf(buf, "obj_info num_cols %d", &mesh->grid_width);
		if (LINE_IS("obj_info num_rows")) sscanf(buf, "obj_info num_rows %d", &mesh->grid_height);
		GET_LINE();
	}
	...
	while (!LINE_IS("end_header") && !LINE_IS("element vertex")) {
		...
		while (LINE_IS("property")) {
			if (!ply_property(buf, elem_len, binary)) return false;
			GET_LINE();
		}
	...
	GET_LINE();
	while (LINE_IS("property")) {
		if (LINE_IS("property float x") || LINE_IS("property float32 x")) vert_pos = vert_len;
		if (LINE_IS("property float nx") || LINE_IS("property float32 nx")) vert_norm = vert_len;
		if (LINE_IS("property uchar diffuse_red") || LINE_IS("property uint8 diffuse_red") ||
		    LINE_IS("property uchar red") || LINE_IS("property uint8 red"))
			vert_color = vert_len;
		if (LINE_IS("property float diffuse_red") || LINE_IS("property float32 diffuse_red") ||
		    LINE_IS("property float red") || LINE_IS("property float32 red"))
			vert_color = vert_len, float_color = true;
		if (LINE_IS("property float confidence") || LINE_IS("property float32 confidence"))
			vert_conf = vert_len;

		if (!ply_property(buf, vert_len, binary)) return false;
	...
	int skip2 = 0;
	while (!LINE_IS("end_header") && !LINE_IS("element face") && !LINE_IS("element tristrips") &&
	       !LINE_IS("element range_grid")) {
		char elem_name[1024];
		int nelem = 0, elem_len = 0;
		sscanf(buf, "element %s %d", elem_name, &nelem);
		GET_LINE();
	...
	if (LINE_IS("element face")) {
		if (sscanf(buf, "element face %d\n", &nfaces) != 1) return false;
		GET_LINE();
		while (LINE_IS("property")) {
			char count_type[256], ind_type[256];
			if (sscanf(buf, "property list %255s %255s vertex_ind", count_type, ind_type) == 2) {
		...
		nstrips = 1;
		GET_LINE();
		if (!LINE_IS("property list int int vertex_ind") && !LINE_IS("property list int32 int32 vertex_ind"))
			return false;
		GET_LINE();
	} else if (LINE_IS("element range_grid")) {
		if (sscanf(buf, "element range_grid %d\n", &ngrid) != 1) return false;
		...
		if (!LINE_IS("property list uchar int vertex_ind") &&
		    !LINE_IS("property list uint8 int32 vertex_ind") && !LINE_IS("property list char int vertex_ind") &&
	...
	while (LINE_IS("property")) {
		if (!ply_property(buf, face_len, binary)) return false;
		GET_LINE();
	}

	// Skip to the end of the header
	while (!LINE_IS("end_header")) GET_LINE();
	if (binary && buf[10] == '\r') {
	...
	if (skip1) {
		if (binary)
			...
			for (int i = 0; i < skip1; i++) GET_WORD();
	}
	if (binary) {
		if (!read_verts_bin(f, mesh, need_swap, nverts, vert_len, vert_pos, vert_norm, vert_color, float_color,
				    vert_conf))
			return false;
	} else {
		if (!read_verts_asc(f, mesh, nverts, vert_len, vert_pos, vert_norm, vert_color, float_color, vert_conf))
			return false;
	}
	...
	if (skip2) {
		if (binary)
			...
			for (int i = 0; i < skip2; i++) GET_WORD();
	}

	if (ngrid) {
		if (binary) {
			if (!read_grid_bin(f, mesh, need_swap)) return false;
		} else {
			if (!read_grid_asc(f, mesh)) return false;
		}
	} else if (nstrips) {
		if (binary) {
			if (!read_strips_bin(f, mesh, need_swap)) return false;
		} else {
			if (!read_strips_asc(f, mesh)) return false;
		}
		mesh->convert_strips(TriMesh::TSTRIP_LENGTH);
	} else if (nfaces) {
		if (binary) {
			if (!read_faces_bin(f, mesh, need_swap, nfaces, face_len, face_count, face_idx)) return false;
		} else {
			if (!read_faces_asc(f, mesh, nfaces, face_len, face_count, face_idx)) return false;
		}
	}
...
static bool read_3ds(FILE* f, TriMesh* mesh) {
	...
	while (1) {
		...
		unsigned chunklen;
		if (!fread(&chunkid, 2, 1, f) || !fread(&chunklen, 4, 1, f)) return (feof(f) != 0);
		if (need_swap) {
			swap_ushort(chunkid);
		...
		switch (chunkid) {
			...
			case CHUNK_3DS_VERT: {
				...
				if (!fread(&nverts, 2, 1, f)) return false;
				if (need_swap) swap_ushort(nverts);
				read_verts_bin(f, mesh, need_swap, nverts, 12, 0, -1, -1, false, -1);
				break;
			}
			case CHUNK_3DS_FACE: {
				unsigned short nfaces;
				if (!fread(&nfaces, 2, 1, f)) return false;
				if (need_swap) swap_ushort(nfaces);
...
static bool read_vvd(FILE* f, TriMesh* mesh) {
	...
	const int skip = 127;
	char buf[skip];
	if (fread(buf, skip, 1, f) != 1) return false;
...
static bool read_ray(FILE* f, TriMesh* mesh) {
	...
	while (!feof(f)) {
		char buf[1024];
		buf[0] = '\0';
		if (fscanf(f, " %1023s", buf) == 0) return true;
		if (LINE_IS("#vertex_num")) {
			...
			int m, nverts;
			if (fscanf(f, " %d %d", &m, &nverts) != 2) return false;
			thisface.clear();
			thisface.reserve(nverts);
			for (int i = 0; i < nverts; i++) {
				int thisv;
				if (fscanf(f, " %d", &thisv) != 1) return false;
...
static bool read_obj(FILE* f, TriMesh* mesh) {
	...
	while (1) {
		...
		} else if (LINE_IS("f ") || LINE_IS("f\t") || LINE_IS("t ") || LINE_IS("t\t")) {
			thisface.clear();
			char* c = buf;
			while (1) {
				while (*c && *c != '\n' && !isspace(*c)) c++;
	...
	// handle it.
	if (mesh->vertices.size() != mesh->normals.size()) mesh->normals.clear();
...
static bool read_off(FILE* f, TriMesh* mesh) {
	...
	if (sscanf(buf, "%d %d %d", &nverts, &nfaces, &unused) < 2) return false;
	if (!read_verts_asc(f, mesh, nverts, 3, 0, -1, -1, false, -1)) return false;
	if (!read_faces_asc(f, mesh, nfaces, 1, 0, 1, true)) return false;
...
static bool read_sm(FILE* f, TriMesh* mesh) {
	...
	if (fscanf(f, "%d", &nverts) != 1) return false;

	if (!read_verts_asc(f, mesh, nverts, 3, 0, -1, -1, false, -1)) return false;

	skip_comments(f);
	if (fscanf(f, "%d", &nfaces) != 1) return true;
	if (!read_faces_asc(f, mesh, nfaces, 0, -1, 0)) return false;
...
static bool read_stl(FILE* f, TriMesh* mesh) {
	...
	for (int i = 0; i < nfacets; i++) {
		...
		if (need_swap) {
			for (int j = 3; j < 12; j++) swap_float(fbuf[j]);
		}
...
static bool read_pts(FILE* f, TriMesh* mesh) {
	while (!feof(f)) {
		...
		int nparsed = sscanf(buf, "%f %f %f %f %f %f", &x, &y, &z, &nx, &ny, &nz);
		if (nparsed >= 3) mesh->vertices.push_back(point(x, y, z));
		if (nparsed == 6) mesh->normals.push_back(vec(nx, ny, nz));
	}
...
static bool read_verts_bin(FILE* f, TriMesh* mesh, bool& need_swap, int nverts, int vert_len, int vert_pos,
	...
	if (nverts < 0 || vert_len < 12 || vert_pos < 0) return false;
	...
	if (have_norm) mesh->normals.resize(new_nverts);
	if (have_color) mesh->colors.resize(new_nverts);
	if (have_conf) mesh->confidences.resize(new_nverts);

	vector<unsigned char> buf(vert_len);
	COND_READ(true, buf[0], vert_len);

	int i = old_nverts;
	memcpy(&mesh->vertices[i][0], &buf[vert_pos], vert_size);
	if (have_norm) memcpy(&mesh->normals[i][0], &buf[vert_norm], norm_size);
	if (have_color && float_color) memcpy(&mesh->colors[i][0], &buf[vert_color], color_size);
	if (have_color && !float_color) mesh->colors[i] = Color(&buf[vert_color]);
	if (have_conf) memcpy(&mesh->confidences[i], &buf[vert_conf], conf_size);
	...
	if (need_swap) {
		...
		if (have_conf) swap_float(mesh->confidences[i]);
	}

	dprintf("\n  Reading %d vertices... ", nverts);
	if (vert_len == 12 && sizeof(point) == 12 && nverts > 1) return slurp_verts_bin(f, mesh, need_swap, nverts);
	while (++i < new_nverts) {
		COND_READ(true, buf[0], vert_len);
		memcpy(&mesh->vertices[i][0], &buf[vert_pos], vert_size);
		if (have_norm) memcpy(&mesh->normals[i][0], &buf[vert_norm], norm_size);
...
static bool read_verts_asc(FILE* f, TriMesh* mesh, int nverts, int vert_len, int vert_pos, int vert_norm,
			   int vert_color, bool float_color, int vert_conf) {
	if (nverts < 0 || vert_len < 3 || vert_pos < 0) return false;
	if (nverts == 0) return true;

	int old_nverts = mesh->vertices.size();
	int new_nverts = old_nverts + nverts;
	mesh->vertices.resize(new_nverts);
	if (vert_norm > 0) mesh->normals.resize(new_nverts);
	if (vert_color > 0) mesh->colors.resize(new_nverts);
	if (vert_conf > 0) mesh->confidences.resize(new_nverts);
	...
	for (int i = old_nverts; i < new_nverts; i++) {
		for (int j = 0; j < vert_len; j++) {
			if (j == vert_pos) {
				if (fscanf(f, "%f %f %f", &mesh->vertices[i][0], &mesh->vertices[i][1],
					   &mesh->vertices[i][2]) != 3)
					return false;
				j += 2;
			} else if (j == vert_norm) {
				if (fscanf(f, "%f %f %f", &mesh->normals[i][0], &mesh->normals[i][1],
					   &mesh->normals[i][2]) != 3)
					return false;
				j += 2;
			} else if (j == vert_color && float_color) {
				float r, g, b;
				if (fscanf(f, "%f %f %f", &r, &g, &b) != 3) return false;
				mesh->colors[i] = Color(r, g, b);
				j += 2;
			} else if (j == vert_color && !float_color) {
				int r, g, b;
				if (fscanf(f, "%d %d %d", &r, &g, &b) != 3) return false;
				mesh->colors[i] = Color(r, g, b);
				j += 2;
			} else if (j == vert_conf) {
				if (fscanf(f, "%f", &mesh->confidences[i]) != 1) return false;
			} else {
				GET_WORD();
			}
		}
...
static bool read_faces_bin(FILE* f, TriMesh* mesh, bool need_swap, int nfaces, int face_len, int face_count,
			   int face_idx) {
	if (nfaces < 0 || face_idx < 0) return false;

	if (nfaces == 0) return true;
	...
	for (int i = 0; i < nfaces; i++) {
		...
		if (face_count >= 0) {
			...
			if (face_idx - face_count == 4) {
				memcpy(&this_ninds, &buf[face_count], sizeof(unsigned));
				if (need_swap) swap_unsigned(this_ninds);
			} else {
		...
		if (need_swap) {
			for (size_t j = 0; j < thisface.size(); j++) swap_int(thisface[j]);
		}
		tess(mesh->vertices, thisface, mesh->faces);
...
static bool read_strips_bin(FILE* f, TriMesh* mesh, bool need_swap) {
	...
	if (need_swap) swap_int(striplen);
	...
	if (need_swap) {
		for (int i = old_striplen; i < new_striplen; i++) swap_int(mesh->tstrips[i]);
	}
...
static bool read_strips_asc(FILE* f, TriMesh* mesh) {
	skip_comments(f);
	int striplen;
	if (fscanf(f, "%d", &striplen) != 1) return false;
	int old_striplen = mesh->tstrips.size();
	int new_striplen = old_striplen + striplen;
	mesh->tstrips.resize(new_striplen);
	...
	for (int i = old_striplen; i < new_striplen; i++)
		if (fscanf(f, "%d", &mesh->tstrips[i]) != 1) return false;
...
static bool read_grid_bin(FILE* f, TriMesh* mesh, bool need_swap) {
	...
	for (int i = 0; i < ngrid; i++) {
		...
		while (n--) {
			if (!fread((void*) &(mesh->grid[i]), 4, 1, f)) return false;
			if (need_swap) swap_int(mesh->grid[i]);
		}
	}
...
static bool read_grid_asc(FILE* f, TriMesh* mesh) {
	...
	for (int i = 0; i < ngrid; i++) {
		int n;
		if (fscanf(f, "%d", &n) != 1) return false;
		while (n--) {
			if (fscanf(f, "%d", &(mesh->grid[i])) != 1) return false;
		}
	}
...
static int ply_type_len(const char* buf, bool binary) {
	if (begins_with(buf, "char") || begins_with(buf, "uchar") || begins_with(buf, "int8") ||
	    begins_with(buf, "uint8")) {
		return 1;
	} else if (begins_with(buf, "short") || begins_with(buf, "ushort") || begins_with(buf, "int16") ||
		   begins_with(buf, "uint16")) {
		return (binary ? 2 : 1);
	} else if (begins_with(buf, "int") || begins_with(buf, "uint") || begins_with(buf, "float") ||
		   begins_with(buf, "int32") || begins_with(buf, "uint32") || begins_with(buf, "float32")) {
		return (binary ? 4 : 1);
	} else if (begins_with(buf, "double") || begins_with(buf, "float64")) {
		return (binary ? 8 : 1);
	}
	return 0;
}
...
static void check_need_swap(const point& p, bool& need_swap) {
	...
	bool makes_sense = (p0 > -BIGNUM && p0 < BIGNUM && p1 > -BIGNUM && p1 < BIGNUM && p2 > -BIGNUM && p2 < BIGNUM);
	if (makes_sense) return;

	swap_float(p0);
	swap_float(p1);
	swap_float(p2);

	bool makes_sense_swapped =
	    (p0 > -BIGNUM && p0 < BIGNUM && p1 > -BIGNUM && p1 < BIGNUM && p2 > -BIGNUM && p2 < BIGNUM);
...
static void check_ind_range(TriMesh* mesh) {
	...
	// All good
	if (min_ind == 0 && max_ind == nv - 1) return;
	...
	if (max_ind - min_ind == nv - 1) {
		...
		for (size_t i = 0; i < mesh->faces.size(); i++)
			for (int j = 0; j < 3; j++) mesh->faces[i][j] -= min_ind;
		return;
	}
...
static void tess(const vector<point>& verts, const vector<int>& thisface, vector<TriMesh::Face>& tris) {
	if (thisface.size() < 3) return;
	if (thisface.size() == 3) {
		tris.push_back(TriMesh::Face(thisface[0], thisface[1], thisface[2]));
		return;
	}
	if (thisface.size() == 4) {
		...
		int i = (d02 < d13) ? 0 : 1;
		tris.push_back(TriMesh::Face(thisface[i], thisface[(i + 1) % 4], thisface[(i + 2) % 4]));
		tris.push_back(TriMesh::Face(thisface[i], thisface[(i + 2) % 4], thisface[(i + 3) % 4]));
		return;
	}

	// 5-gon or higher - just tesselate arbitrarily...
	for (size_t i = 2; i < thisface.size(); i++)
		tris.push_back(TriMesh::Face(thisface[0], thisface[i - 1], thisface[i]));
}
...
bool TriMesh::write(const char* filename) {
	...
	enum { PLY_ASCII, PLY_BINARY_BE, PLY_BINARY_LE, RAY, OBJ, OFF, SM, STL, PTS, CC, DAE } filetype;
	...
	switch (filetype) {
		case PLY_ASCII:
			...
			break;
		case PLY_BINARY_BE:
			ok = write_ply_binary(this, f, false, write_norm, write_grid, float_color);
			break;
		case PLY_BINARY_LE:
			ok = write_ply_binary(this, f, true, write_norm, write_grid, float_color);
			break;
		case RAY:
			ok = write_ray(this, f);
			break;
...
// Write a ply header
static bool write_ply_header(TriMesh* mesh, FILE* f, const char* format, bool write_grid, bool write_tstrips,
...
static bool write_ply_ascii(TriMesh* mesh, FILE* f, bool write_norm, bool write_grid, bool float_color) {
	if (write_norm) mesh->need_normals();

	bool write_tstrips = !write_grid && !mesh->tstrips.empty();

	if (!write_ply_header(mesh, f, "ascii", write_grid, write_tstrips, write_norm, float_color)) return false;
	if (!write_verts_asc(mesh, f, "", write_norm ? " " : 0, " ", float_color, " ", "")) return false;
	if (write_grid) {
...
static bool write_ply_binary(TriMesh* mesh, FILE* f, bool little_endian, bool write_norm, bool write_grid,
	...
	const char* format = little_endian ? "binary_little_endian" : "binary_big_endian";
	if (!write_ply_header(mesh, f, format, write_grid, write_tstrips, write_norm, float_color)) return false;
	if (!write_verts_bin(mesh, f, need_swap, write_norm, true, float_color, true)) return false;
	if (write_grid) {
		return write_grid_bin(mesh, f, need_swap);
	} else if (write_tstrips) {
		int s = mesh->tstrips.size();
		if (need_swap) swap_int(s);
		FWRITE(&s, 4, 1, f);
		mesh->convert_strips(TriMesh::TSTRIP_TERM);
...
static bool write_ray(TriMesh* mesh, FILE* f) {
	...
	mesh->need_normals();
	if (!write_verts_asc(mesh, f, "#vertex ", "  ", 0, false, 0, "  0 0")) return false;
	mesh->need_faces();
	...
	if (!write_norm) {
		...
		for (size_t i = 0; i < mesh->faces.size(); i++) {
			int n = fprintf(f, "f %d//%d %d//%d %d//%d\n", mesh->faces[i][0], mesh->faces[i][0],
					mesh->faces[i][1], mesh->faces[i][1], mesh->faces[i][2], mesh->faces[i][2]);
			if (n < 6) {
				ok = false;
				break;
			}
...
static bool write_off(TriMesh* mesh, FILE* f) {
	...
	return write_verts_asc(mesh, f, "", 0, 0, false, 0, "") && write_faces_asc(mesh, f, "3 ", "");
}

// Write an SM file
static bool write_sm(TriMesh* mesh, FILE* f) {
	FPRINTF(f, "%zu\n", mesh->vertices.size());
	if (!write_verts_asc(mesh, f, "", 0, 0, false, 0, "")) return false;
	mesh->need_faces();
	FPRINTF(f, "%zu\n", mesh->faces.size());
	if (!write_faces_asc(mesh, f, "", "")) return false;
	FPRINTF(f, "0 0\n");
	return true;
}
...
static bool write_stl(TriMesh* mesh, FILE* f) {
	...
	for (size_t i = 0; i < mesh->faces.size(); i++) {
		...
		if (need_swap) {
			for (int j = 0; j < 12; j++) swap_float(fbuf[j]);
		}
		FWRITE(fbuf, 48, 1, f);
...
static bool write_cc(TriMesh* mesh, FILE* f, const char* filename, bool write_norm, bool float_color) {
	...
	if (write_norm) {
		...
		for (int i = 0; i < nv; i++) {
			FPRINTF(f, "\t\t{ %.7g, %.7g, %.7g },\n", mesh->normals[i][0], mesh->normals[i][1],
				mesh->normals[i][2]);
		}
		FPRINTF(f, "\t};\n");
	}
	if (!mesh->colors.empty() && float_color) {
		...
		for (int i = 0; i < nv; i++) {
			FPRINTF(f, "\t\t{ %.7g, %.7g, %.7g },\n", mesh->colors[i][0], mesh->colors[i][1],
				mesh->colors[i][2]);
		}
		FPRINTF(f, "\t};\n");
	}
	if (!mesh->colors.empty() && !float_color) {
		...
		for (int i = 0; i < nv; i++) {
			FPRINTF(f, "\t\t{ %d, %d, %d },\n", color2uchar(mesh->colors[i][0]),
				color2uchar(mesh->colors[i][1]), color2uchar(mesh->colors[i][2]));
		}
		FPRINTF(f, "\t};\n");
	}
...
static bool write_verts_asc(TriMesh* mesh, FILE* f, const char* before_vert, const char* before_norm,
			    ...
			    const char* after_line) {
	for (size_t i = 0; i < mesh->vertices.size(); i++) {
		FPRINTF(f, "%s%.7g %.7g %.7g", before_vert, mesh->vertices[i][0], mesh->vertices[i][1],
			mesh->vertices[i][2]);
		if (!mesh->normals.empty() && before_norm)
			FPRINTF(f, "%s%.7g %.7g %.7g", before_norm, mesh->normals[i][0], mesh->normals[i][1],
				mesh->normals[i][2]);
		if (!mesh->colors.empty() && before_color && float_color)
			FPRINTF(f, "%s%.7g %.7g %.7g", before_color, mesh->colors[i][0], mesh->colors[i][1],
				mesh->colors[i][2]);
		if (!mesh->colors.empty() && before_color && !float_color)
			FPRINTF(f, "%s%d %d %d", before_color, color2uchar(mesh->colors[i][0]),
				color2uchar(mesh->colors[i][1]), color2uchar(mesh->colors[i][2]));
		if (!mesh->confidences.empty() && before_conf) FPRINTF(f, "%s%.7g", before_conf, mesh->confidences[i]);
		FPRINTF(f, "%s\n", after_line);
	}
	return true;
}
...
static void swap_vert_props(TriMesh* mesh, bool swap_color) {
	...
	if (!mesh->confidences.empty()) {
		for (size_t i = 0; i < mesh->confidences.size(); i++) swap_float(mesh->confidences[i]);
	}
}

// Helper for write_verts_bin: actually does the writing.
static bool write_verts_bin_helper(TriMesh* mesh, FILE* f, bool write_norm, bool write_color, bool float_color,
				   bool write_conf) {
	if ((mesh->normals.empty() || !write_norm) && (mesh->colors.empty() || !write_color) &&
		...
		for (size_t i = 0; i < mesh->vertices.size(); i++) {
			...
			if (!mesh->normals.empty() && write_norm) FWRITE(&(mesh->normals[i][0]), 12, 1, f);
			if (!mesh->colors.empty() && write_color && float_color)
				FWRITE(&(mesh->colors[i][0]), 12, 1, f);
			if (!mesh->colors.empty() && write_color && !float_color) {
				unsigned char c[3] = {color2uchar(mesh->colors[i][0]), color2uchar(mesh->colors[i][1]),
						      color2uchar(mesh->colors[i][2])};
				FWRITE(&c, 3, 1, f);
			}
			if (!mesh->confidences.empty() && write_conf) FWRITE(&(mesh->confidences[i]), 4, 1, f);
		}
	}
	return true;
}
...
static bool write_verts_bin(TriMesh* mesh, FILE* f, bool need_swap, bool write_norm, bool write_color, bool float_color,
			    bool write_conf) {
	if (need_swap) swap_vert_props(mesh, float_color);
	bool ok = write_verts_bin_helper(mesh, f, write_norm, write_color, float_color, write_conf);
	if (need_swap) swap_vert_props(mesh, float_color);
	return ok;
}
...
static bool write_faces_asc(TriMesh* mesh, FILE* f, const char* before_face, const char* after_line) {
	mesh->need_faces();
	for (size_t i = 0; i < mesh->faces.size(); i++) {
		FPRINTF(f, "%s%d %d %d%s\n", before_face, mesh->faces[i][0], mesh->faces[i][1], mesh->faces[i][2],
			after_line);
	}
	return true;
}
...
static bool write_strips_bin(TriMesh* mesh, FILE* f, bool need_swap) {
	if (need_swap) {
		for (size_t i = 0; i < mesh->tstrips.size(); i++) swap_int(mesh->tstrips[i]);
	}
...
void TriMesh::dprintf(const char* format, ...) {
	...
	va_list ap;
	va_start(ap, format);
	size_t required = 1 + vsnprintf(nullptr, 0, format, ap);
	va_end(ap);
