...
static void find_overlap_onedir(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, const KDtree* kd2,
				float& overlap_area, float& nonoverlap_area, float& rmsdist) {
	overlap_area = 0.0f;
	rmsdist = 0.0f;
	...
	for (int i = 0; i < nsamp; i++) {
		...
		if (mesh2->is_bdy(ind2)) continue;
		if (dot(xf12r * mesh1->normals[ind], mesh2->normals[ind2]) <= 0.0f) continue;
		overlap_area += this_area;
		rmsdist += this_area * sqr(dot(p - point(q), mesh2->normals[ind2]));
	}

	float total_area = mesh1->stat(TriMesh::STAT_SUM, TriMesh::STAT_FACEAREA);
...
void find_overlap(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, const KDtree* kd1,
	...
	if (area) rmsdist = 0.5f * (rmsdist1 + rmsdist2);
}

// Return intersection-over-union between mesh1 and mesh2.
float iou(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, const KDtree* kd1, const KDtree* kd2) {
	mesh1->need_normals();
	mesh1->need_neighbors();
	mesh1->need_adjacentfaces();
	mesh1->need_pointareas();
