...
struct AccumVec {
	...
	inline void operator()(const TriMesh*, int /* v0 */, T& f, float w, int v) const { f += w * field[v]; }
};

struct AccumCurv {
	inline void operator()(const TriMesh* themesh, int v0, vec& c, float w, int v) const {
		vec ncurv;
		proj_curv(themesh->pdir1[v], themesh->pdir2[v], themesh->curv1[v], 0, themesh->curv2[v],
			  themesh->pdir1[v0], themesh->pdir2[v0], ncurv[0], ncurv[1], ncurv[2]);
		c += w * ncurv;
	}
};

struct AccumDCurv {
	inline void operator()(const TriMesh* themesh, int v0, Vec<4>& d, float w, int v) const {
		Vec<4> ndcurv;
		proj_dcurv(themesh->pdir1[v], themesh->pdir2[v], themesh->dcurv[v], themesh->pdir1[v0],
			   themesh->pdir2[v0], ndcurv);
		d += w * ndcurv;
	}
};
...
static void diffuse_vert_field(TriMesh* themesh, vector<unsigned>& flags, unsigned& flag_curr, const ACCUM& accum,
	...
	while (!boundary.empty()) {
		int n = boundary.back();
		boundary.pop_back();
		if (flags[n] == flag_curr) continue;
		flags[n] = flag_curr;
		if (dot(nv, themesh->normals[n]) <= 0.0f) continue;
		// Gaussian weight
		float w = wt(themesh, n, v, invsigma2);
		if (w == 0.0f) continue;
		// Downweight things pointing in different directions
		w *= dot(nv, themesh->normals[n]);
		...
		for (size_t i = 0; i < themesh->neighbors[n].size(); i++) {
			...
			if (flags[nn] == flag_curr) continue;
...
#pragma omp for
		for (int i = 0; i < nv; i++) {
			diffuse_vert_field(themesh, flags, flag_curr, AccumVec<vec>(themesh->vertices), i, invsigma2,
					   dflt[i]);
...
#pragma omp for
		for (int i = 0; i < nf; i++) {
			point c = (themesh->vertices[themesh->faces[i][0]] + themesh->vertices[themesh->faces[i][1]] +
				   themesh->vertices[themesh->faces[i][2]]) *
				  (1.0f / 3.0f);
			for (int j = 0; j < 3; j++) {
				int v = themesh->faces[i][j];
				vec d = 0.5f * (c - themesh->vertices[v]);
				dflt[v] += themesh->cornerareas[i][j] / themesh->pointareas[themesh->faces[i][j]] *
...
#pragma omp for
		for (int i = 0; i < nv; i++) {
			diffuse_vert_field(themesh, flags, flag_curr, AccumVec<point>(dflt), i, invsigma2, dflt2[i]);
		}

		// Update vertex positions
#pragma omp for
		for (int i = 0; i < nv; i++) themesh->vertices[i] += dflt[i] - dflt2[i];  // second Laplacian
	}  // #pragma omp parallel
...
static void jones_filter(TriMesh* themesh, vector<unsigned>& flags, unsigned& flag_curr, int v, float invsigma2_1,
			 float invsigma2_2, vector<point>& oldverts) {
	...
	while (!boundary.empty()) {
		...
		point prediction = q + norm * dot(p - q, norm);
...
#pragma omp for
		for (int i = 0; i < nv; i++)
			jones_filter(themesh, flags, flag_curr, i, invsigma2_1, invsigma2_2, oldverts);
	}

	dprintf("Done.  Filtering took %f sec.\n", now() - t);
	themesh->normals.clear();
	if (had_normals) themesh->need_normals();
}
...
#pragma omp for
		for (int i = 0; i < nv; i++) diffuse_vert_field(themesh, flags, flag_curr, a, i, invsigma2, flt[i]);
	}  // #pragma omp parallel
...
#pragma omp for
		for (int i = 0; i < nv; i++) {
			diffuse_vert_field(themesh, flags, flag_curr, a, i, invsigma2, nflt[i]);
			normalize(nflt[i]);
		}
	}  // #pragma omp parallel
...
#pragma omp for
		for (int i = 0; i < nv; i++)
			diffuse_vert_field(themesh, flags, flag_curr, AccumCurv(), i, invsigma2, cflt[i]);

#pragma omp for
		for (int i = 0; i < nv; i++)
			diagonalize_curv(themesh->pdir1[i], themesh->pdir2[i], cflt[i][0], cflt[i][1], cflt[i][2],
					 themesh->normals[i], themesh->pdir1[i], themesh->pdir2[i], themesh->curv1[i],
					 themesh->curv2[i]);
	}  // #pragma omp parallel
...
#pragma omp for
		for (int i = 0; i < nv; i++)
			diffuse_vert_field(themesh, flags, flag_curr, AccumDCurv(), i, invsigma2, dflt[i]);
	}  // #pragma omp parallel
