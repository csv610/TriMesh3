...
void TriMesh::need_pointareas() {
	...
	if (pointareas.size() == vertices.size()) return;
	need_faces();
...
#pragma omp parallel for
	for (int i = 0; i < nf; i++) {
		...
		float area = 0.5f * len(cross(e[0], e[1]));
		float l2[3] = {len2(e[0]), len2(e[1]), len2(e[2])};

		// Barycentric weights of circumcenter
		float bcw[3] = {l2[0] * (l2[1] + l2[2] - l2[0]), l2[1] * (l2[2] + l2[0] - l2[1]),
				l2[2] * (l2[0] + l2[1] - l2[2])};
		if (bcw[0] <= 0.0f) {
			...
			for (int j = 0; j < 3; j++) cornerareas[i][j] = scale * (bcw[NEXT_MOD3(j)] + bcw[PREV_MOD3(j)]);
		}
#pragma omp atomic
