...
//
static inline GLint myGluProject(GLdouble objX, GLdouble objY, GLdouble objZ, const GLdouble* model,
...
static inline GLint myGluUnProject(GLdouble winX, GLdouble winY, GLdouble winZ, const GLdouble* model,
				   const GLdouble* proj, const GLint* view, GLdouble* objX, GLdouble* objY,
				   GLdouble* objZ) {
	xform xf = inv(xform(proj) * xform(model));
	dvec3 v = xf * dvec3((winX - view[0]) / view[2] * 2.0 - 1.0, (winY - view[1]) / view[3] * 2.0 - 1.0,
			     winZ * 2.0 - 1.0);
	*objX = v[0];
	*objY = v[1];
	*objZ = v[2];
...
void GLCamera::startspin() {
	...
	if (dt < SPIN_TIME && fabs(spinspeed) > SPIN_SPEED) dospin = true;
}
...
void GLCamera::rotate(int mousex, int mousey, xform& xf) {
	...
	spinaxis = cross(pos1, pos2);
	float spinamount = sqrt(sqr(nx - ox) + sqr(ny - oy));

	xf = xform::trans(spincenter) * xform::rot(spinamount, spinaxis) * xform::trans(-spincenter) * xf;

	float dt = now() - last_time;
	if (dt > SPIN_TIME)
		spinspeed = spinamount / SPIN_TIME;
	else
		spinspeed = (spinamount / SPIN_TIME) + (1.0f - dt / SPIN_TIME) * spinspeed;
}
...
void GLCamera::movez(int mousex, int mousey, xform& xf) {
	float delta = MOVEZ_MULT / field_of_view * pixscale * ((mousex - lastmousex) - (mousey - lastmousey));
...
void GLCamera::wheel(Mouse::button updown, xform& xf) {
	...
	if (updown == Mouse::WHEELDOWN) dz = -dz;
	// xf = xform::trans(0, 0, -dz) * xf;
	xf = xform::trans(dz * spincenter / len(spincenter)) * xf;
...
void GLCamera::mouse_click(int mousex, int mousey, const point& scene_center, float scene_size) {
	GLdouble M[16], P[16];
	GLint V[4];
#if 0
	glGetDoublev(GL_MODELVIEW_MATRIX, M);
	...
	point surface_point;
	if (read_depth(mousex, mousey, surface_point)) click_depth = -surface_point[2];

	GLdouble cx = 0.0, cy = 0.0, cz = 0.0;
	myGluProject(scene_center[0], scene_center[1], scene_center[2], M, P, V, &cx, &cy, &cz);
	...
	GLdouble sx, sy, sz;
	myGluUnProject((xmin + xmax) / 2, (ymin + ymax) / 2, 1, M, P, V, &sx, &sy, &sz);
	...
	myGluProject(spincenter[0], spincenter[1], spincenter[2], M, P, V, &cx, &cy, &cz);
...
void GLCamera::mouse(int mousex, int mousey, Mouse::button b, const point& scene_center, float scene_size, xform& xf) {
	if (b == Mouse::NONE && lastb == Mouse::NONE) return;
	...
	dospin = false;
	if ((b != lastb) && (b != Mouse::NONE)) mouse_click(mousex, mousey, scene_center, scene_size);

	// Handle rotation
	if ((b == Mouse::ROTATE) && (lastb == Mouse::NONE)) spinspeed = 0;
	if ((b == Mouse::ROTATE) && (lastb == Mouse::ROTATE)) rotate(mousex, mousey, xf);
	if ((b == Mouse::NONE) && (lastb == Mouse::ROTATE)) startspin();

	// Handle translation
	if ((b == Mouse::MOVEXY) && (lastb == Mouse::MOVEXY)) movexy(mousex, mousey, xf);
	if ((b == Mouse::MOVEZ) && (lastb == Mouse::MOVEZ)) movez(mousex, mousey, xf);
	if (b == Mouse::WHEELUP || b == Mouse::WHEELDOWN) wheel(b, xf);

	// Handle lighting
	if (b == Mouse::LIGHT) relight(mousex, mousey);
...
void GLCamera::setupGL(const point& scene_center, float scene_size) const {
	...
	fardist = max(-(scene_center[2] - scene_size), scene_size / 10.0f);
	neardist = max(-(scene_center[2] + scene_size), fardist / MAX_FAR_NEAR_RATIO);
