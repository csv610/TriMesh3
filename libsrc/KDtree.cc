...
static inline float dist2ray2(const float* x, const float* p, const float* d) {
	...
	return sqr(xp0) + sqr(xp1) + sqr(xp2) - sqr(xp0 * d[0] + xp1 * d[1] + xp2 * d[2]);
}
...
void KDtree::Node::build(KDtree* kd, const float** pts, size_t n) {
	...
	for (size_t i = 1; i < n; i++) {
		if (pts[i][0] < xmin)
			xmin = pts[i][0];
		else if (pts[i][0] > xmax)
			xmax = pts[i][0];
		if (pts[i][1] < ymin)
			ymin = pts[i][1];
		else if (pts[i][1] > ymax)
			ymax = pts[i][1];
		if (pts[i][2] < zmin)
			zmin = pts[i][2];
		else if (pts[i][2] > zmax)
			zmax = pts[i][2];
	}
	...
	node.splitaxis = 2;
	if (rx > ry) {
		if (rx > rz) node.splitaxis = 0;
	} else {
		if (ry > rz) node.splitaxis = 1;
	}
...
crossed:
	...
	less--;
	left--;
	while (less >= pts) swap(*less--, *left--);

	greater++;
	right++;
	while (greater <= last) swap(*greater++, *right++);
	...
	size_t split_left = (num_leq <= ideal_split) ? num_leq : (num_less >= ideal_split) ? num_less : ideal_split;
...
void KDtree::Node::find_closest_to_pt(KDtree::Node::Traversal_Info& ti) const {
	...
	if (npts) {
		for (int i = 0; i < npts; i++) {
			...
			if (myd2 < ti.closest_d2 && leaf.p[i] != ti.p) {
				...
				ti.closest_d2 = myd2;
				ti.closest_d = ti.approx_multiplier * sqrt(ti.closest_d2);
				ti.closest = leaf.p[i];
			}
		}
		return;
	...
	if (dist2(node.center, ti.p) >= sqr(node.r + ti.closest_d)) return;
	...
	if (myd >= 0.0f) {
		...
		if (myd < ti.closest_d) node.child2->find_closest_to_pt(ti);
	} else {
		node.child2->find_closest_to_pt(ti);
		if (-myd < ti.closest_d) node.child1->find_closest_to_pt(ti);
	}
}
...
void KDtree::Node::find_closest_compat_to_pt(KDtree::Node::Traversal_Info& ti) const {
	if (npts) {
		for (int i = 0; i < npts; i++) {
			float myd2 = dist2(leaf.p[i], ti.p);
			if (myd2 < ti.closest_d2 && leaf.p[i] != ti.p && (*ti.iscompat)(leaf.p[i])) {
				ti.closest_d2 = myd2;
				ti.closest_d = ti.approx_multiplier * sqrt(ti.closest_d2);
				ti.closest = leaf.p[i];
			}
	...
	if (myd >= 0.0f) {
		...
		if (myd < ti.closest_d) node.child2->find_closest_compat_to_pt(ti);
	} else {
		node.child2->find_closest_compat_to_pt(ti);
		if (-myd < ti.closest_d) node.child1->find_closest_compat_to_pt(ti);
	}
}
...
void KDtree::Node::find_closest_to_ray(KDtree::Node::Traversal_Info& ti) const {
	...
	// Check whether to abort
	if (dist2ray2(node.center, ti.p, ti.dir) >= sqr(node.r + ti.closest_d)) return;
...
void KDtree::Node::find_k_closest_to_pt(KDtree::Node::Traversal_Info& ti) const {
	...
	if (npts) {
		for (int i = 0; i < npts; i++) {
			float myd2 = dist2(leaf.p[i], ti.p);
			if ((myd2 < ti.closest_d2 || ti.knn.size() < ti.k) && leaf.p[i] != ti.p) {
				float myd = sqrt(myd2);
				...
				// See earlier comment for how approx works
				ti.closest_d = ti.approx_multiplier * ti.knn[0].first;
	...
	// Check whether to abort
	if (dist2(node.center, ti.p) >= sqr(node.r + ti.closest_d) && ti.knn.size() == ti.k) return;

	// Recursive case - pick the optimal order
	float myd = node.center[node.splitaxis] - ti.p[node.splitaxis];
	if (myd >= 0.0f) {
		node.child1->find_k_closest_to_pt(ti);
		if (myd < ti.closest_d || ti.knn.size() != ti.k) node.child2->find_k_closest_to_pt(ti);
	} else {
		node.child2->find_k_closest_to_pt(ti);
		if (-myd < ti.closest_d || ti.knn.size() != ti.k) node.child1->find_k_closest_to_pt(ti);
	}
}
...
void KDtree::Node::find_k_closest_compat_to_pt(KDtree::Node::Traversal_Info& ti) const {
	...
	if (myd >= 0.0f) {
		...
		if (myd < ti.closest_d || ti.knn.size() != ti.k) node.child2->find_k_closest_compat_to_pt(ti);
	} else {
		node.child2->find_k_closest_compat_to_pt(ti);
		if (-myd < ti.closest_d || ti.knn.size() != ti.k) node.child1->find_k_closest_compat_to_pt(ti);
	}
}
...
bool KDtree::Node::exists_pt(KDtree::Node::Traversal_Info& ti) const {
	...
	if (npts) {
		for (int i = 0; i < npts; i++) {
			float myd2 = dist2(leaf.p[i], ti.p);
			if (myd2 < ti.closest_d2 && leaf.p[i] != ti.p) return true;
		}
		return false;
	}

	// Check whether to abort
	if (dist2(node.center, ti.p) >= sqr(node.r + ti.closest_d)) return false;

	// Recursive case - pick the optimal order
	float myd = node.center[node.splitaxis] - ti.p[node.splitaxis];
	if (myd >= 0.0f) {
		if (node.child1->exists_pt(ti)) return true;
		return node.child2->exists_pt(ti);
	} else {
		if (node.child2->exists_pt(ti)) return true;
...
void KDtree::build(const float* ptlist, size_t n) {
	...
	for (size_t i = 0; i < n; i++) pts[i] = ptlist + i * 3;
...
const float* KDtree::closest_to_ray(const float* p, const float* dir, float maxdist2 /* = 0.0f */,
				    const CompatFunc* iscompat /* = nullptr */, float approx_eps /* = 0.0f */) const {
	if (!root || !p || !dir) return nullptr;

	float one_over_dir_len = 1.0f / sqrt(sqr(dir[0]) + sqr(dir[1]) + sqr(dir[2]));
	float normalized_dir[3] = {dir[0] * one_over_dir_len, dir[1] * one_over_dir_len, dir[2] * one_over_dir_len};
	Node::Traversal_Info ti;
...
void KDtree::find_k_closest_to_pt(std::vector<const float*>& knn, int k, const float* p, float maxdist2 /* = 0.0f */,
				  const CompatFunc* iscompat /* = nullptr */, float approx_eps /* = 0.0f */) const {
	knn.clear();
	if (!root || !p) return;
	...
	for (size_t i = 0; i < found; i++) knn[i] = ti.knn[i].second;
}
