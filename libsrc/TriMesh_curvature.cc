...
static void rot_coord_sys(const vec& old_u, const vec& old_v, const vec& new_norm, vec& new_u, vec& new_v) {
	new_u = old_u;
	new_v = old_v;
	vec old_norm = cross(old_u, old_v);
...
// new_u, new_v basis.
void proj_curv(const vec& old_u, const vec& old_v, float old_ku, float old_kuv, float old_kv, const vec& new_u,
...
void proj_dcurv(const vec& old_u, const vec& old_v, const Vec<4> old_dcurv, const vec& new_u, const vec& new_v,
		Vec<4>& new_dcurv) {
	vec r_new_u, r_new_v;
	rot_coord_sys(new_u, new_v, cross(old_u, old_v), r_new_u, r_new_v);
	...
	new_dcurv[0] = old_dcurv[0] * u1 * u1 * u1 + old_dcurv[1] * 3.0f * u1 * u1 * v1 +
		       old_dcurv[2] * 3.0f * u1 * v1 * v1 + old_dcurv[3] * v1 * v1 * v1;
	new_dcurv[1] = old_dcurv[0] * u1 * u1 * u2 + old_dcurv[1] * (u1 * u1 * v2 + 2.0f * u2 * u1 * v1) +
		       old_dcurv[2] * (u2 * v1 * v1 + 2.0f * u1 * v1 * v2) + old_dcurv[3] * v1 * v1 * v2;
	new_dcurv[2] = old_dcurv[0] * u1 * u2 * u2 + old_dcurv[1] * (u2 * u2 * v1 + 2.0f * u1 * u2 * v2) +
		       old_dcurv[2] * (u1 * v2 * v2 + 2.0f * u2 * v2 * v1) + old_dcurv[3] * v1 * v2 * v2;
	new_dcurv[3] = old_dcurv[0] * u2 * u2 * u2 + old_dcurv[1] * 3.0f * u2 * u2 * v2 +
		       old_dcurv[2] * 3.0f * u2 * v2 * v2 + old_dcurv[3] * v2 * v2 * v2;
}

// Given a curvature tensor, find principal directions and curvatures
// Makes sure that pdir1 and pdir2 are perpendicular to normal
void diagonalize_curv(const vec& old_u, const vec& old_v, float ku, float kuv, float kv, const vec& new_norm,
...
void TriMesh::need_curvatures() {
	...
	if (curv1.size() == vertices.size()) return;
	need_faces();
	need_normals();
	need_pointareas();
...
#pragma omp parallel for
	for (int i = 0; i < nv; i++) {
		pdir1[i] = cross(pdir1[i], normals[i]);
		normalize(pdir1[i]);
		pdir2[i] = cross(normals[i], pdir1[i]);
	}
...
#pragma omp parallel for
	for (int i = 0; i < nf; i++) {
		...
		for (int j = 0; j < 3; j++) {
			...
			// w[1][1] += v*v + u*u;
			// w[1][2] += u*v;
			vec dn = normals[faces[i][PREV_MOD3(j)]] - normals[faces[i][NEXT_MOD3(j)]];
			float dnu = dot(dn, t);
			float dnv = dot(dn, b);
			m[0] += dnu * u;
			m[1] += dnu * v + dnv * u;
...
#pragma omp parallel for
	for (int i = 0; i < nv; i++) {
		diagonalize_curv(pdir1[i], pdir2[i], curv1[i], curv12[i], curv2[i], normals[i], pdir1[i], pdir2[i],
				 curv1[i], curv2[i]);
	}
	dprintf("Done.\n");
}
...
void TriMesh::need_dcurv() {
	std::lock_guard<std::recursive_mutex> lock(computation_mutex);
	if (dcurv.size() == vertices.size()) return;
	need_curvatures();
...
#pragma omp parallel for
	for (int i = 0; i < nf; i++) {
		...
		for (int j = 0; j < 3; j++) {
			int vj = faces[i][j];
			proj_curv(pdir1[vj], pdir2[vj], curv1[vj], 0, curv2[vj], t, b, fcurv[j][0], fcurv[j][1],
				  fcurv[j][2]);
		}
