...
TriMesh* make_plane(int tess_x, int tess_y) {
	if (tess_x < 1) tess_x = 1;
	if (tess_y < 1) tess_y = tess_x;
...
TriMesh* make_disc(int tess_th, int tess_r) {
	if (tess_th < 3) tess_th = 3;
	if (tess_r < 1) tess_r = 1;
	...
	for (int i = 0; i < tess_th; i++) mkface(mesh, 0, i + 1, ((i + 1) % tess_th) + 1);
	for (int j = 1; j < tess_r; j++) {
		int base = 1 + (j - 1) * tess_th;
		for (int i = 0; i < tess_th; i++) {
			int i1 = (i + 1) % tess_th;
			mkquad(mesh, base + tess_th + i, base + tess_th + i1, base + i, base + i1);
		}
	}
...
TriMesh* make_ccyl(int tess_th, int tess_h, float r) {
	...
	for (int i = 0; i < tess_th; i++) mkface(mesh, 0, ((i + 1) % tess_th) + 1, i + 1);
	for (int j = 1; j < tess_h; j++) {
		int base = 1 + (j - 1) * tess_th;
		for (int i = 0; i < tess_th; i++) {
			int i1 = (i + 1) % tess_th;
	...
	for (int i = 0; i < tess_th; i++) mkface(mesh, base + i, base + ((i + 1) % tess_th), base + tess_th);
...
TriMesh* make_torus(int tess_th, int tess_ph, float r) {
	if (tess_th < 3) tess_th = 3;
	if (tess_ph < 3) tess_ph = 3;

	TriMesh* mesh = make_cyl(tess_ph, tess_th);

	for (int i = 0; i < tess_ph; i++) mesh->vertices.pop_back();
...
TriMesh* make_knot(int tess_th, int tess_ph, float r) {
	...
	for (int j = 0; j < tess_th; j++) {
		float th = M_2PIf * j / tess_th;
		vec pos(2.0f * sin(2.0f * th) + cos(th), 2.0f * cos(2.0f * th) + sin(th), cos(3.0f * th));
		pos /= 3.0f;
		vec vel(4.0f * cos(2.0f * th) - sin(th), -4.0f * sin(2.0f * th) + cos(th), -3.0f * sin(3.0f * th));
		normalize(vel);
		for (int i = 0; i < tess_ph; i++) {
			float ph = M_2PIf * i / tess_ph;
			vec u = cross(vel, vec(0, 0, 1));
			normalize(u);
			vec v = cross(u, vel);
...
TriMesh* make_klein(int tess_th, int tess_ph) {
	...
	for (int j = 0; j < tess_th; j++) {
		...
		int part = 0;
		for (int i = 0; i < 7; i++)
			if (v > M_PIf) v -= M_PIf, part++;
		float flipx = 1.0f;
		if (part > 3) part -= 4, flipx = -1.0f;
		if (part == 0 || part == 2) v = M_PIf - v;
		for (int i = 0; i < tess_ph; i++) {
			float u = M_2PIf * i / tess_ph;
			// Based on formulae from
			// http://emsh.calarts.edu/~mathart/sw/klein/Klein.html
			if (part == 0)
				mesh->vertices[i + j * tess_ph].set((2.5f + 1.5f * cos(v)) * flipx * cos(u),
								    (2.5f + 1.5f * cos(v)) * sin(u), -2.5f * sin(v));
			else if (part == 1)
				mesh->vertices[i + j * tess_ph].set((2.5f + 1.5f * cos(v)) * flipx * cos(u),
								    (2.5f + 1.5f * cos(v)) * sin(u), 3.0f * v);
			else if (part == 2)
				mesh->vertices[i + j * tess_ph].set(2.0f + (2.0f - flipx * cos(u)) * cos(v), sin(u),
								    3.0f * M_PIf + (2.0f - flipx * cos(u)) * sin(v));
			else
				mesh->vertices[i + j * tess_ph].set(2.0f + 2.0f * cos(v) - flipx * cos(u), sin(u),
								    3.0f * (M_PIf - v));
		}
	}
	return mesh;
...
TriMesh* make_helix(int tess_th, int tess_ph, float turns, float r) {
	...
	for (int j = 0; j <= tess_th; j++) {
		...
		for (int i = 0; i < tess_ph; i++) {
			float ph = M_2PIf * i / tess_ph;
			mesh->vertices[i + j * tess_ph] = helixpos + r * cos(ph) * xdir + r * sin(ph) * ydir;
		}
	}
	return mesh;
}
...
TriMesh* make_sphere_polar(int tess_ph, int tess_th) {
	...
	for (int i = 0; i < tess_ph; i++) mkface(mesh, 0, ((i + 1) % tess_ph) + 1, i + 1);

	for (int j = 0; j < tess_th - 2; j++) {
		int base = 1 + j * tess_ph;
		for (int i = 0; i < tess_ph; i++) {
			int i1 = (i + 1) % tess_ph;
			mkquad(mesh, base + i, base + i1, base + tess_ph + i, base + tess_ph + i1);
		}
	}

	int base = 1 + (tess_th - 2) * tess_ph;
	for (int i = 0; i < tess_ph; i++) mkface(mesh, base + i, base + ((i + 1) % tess_ph), base + tess_ph);
...
TriMesh* make_sphere_subdiv(int nfaces, int nsubdiv) {
	...
	for (size_t i = 0; i < mesh->vertices.size(); i++) normalize(mesh->vertices[i]);
	for (int i = 0; i < nsubdiv; i++) subdiv(mesh);
	for (size_t i = 0; i < mesh->vertices.size(); i++) normalize(mesh->vertices[i]);
	return mesh;
}
...
#undef ASSIGN
	...
	for (int i = 0; i < nverts; i++) mesh->vertices[i] *= scale;
	...
	while (i < face_data_size) {
		...
		if (face_size == 3) {
			mkface(mesh, face_data[i], face_data[i + 1], face_data[i + 2]);
			i += 3;
			continue;
		}
		point center;
		for (int j = 0; j < face_size; j++) center += mesh->vertices[face_data[i + j]];
		center *= 1.0f / face_size;
		int center_ind = mesh->vertices.size();
		mesh->vertices.push_back(center);
		for (int j = 0; j < face_size; j++)
			mkface(mesh, center_ind, face_data[i + j], face_data[i + (j + 1) % face_size]);
		i += face_size;
	}
...
TriMesh* make_surface_of_revolution(int tess_th, const vector<point>& curve_pts) {
	...
	for (int i = 0; i < tess_th; i++) {
		...
		for (int j = 0; j < n; j++) mesh->vertices[i + j * tess_th] = xf * curve_pts[j];
	}
	...
	for (int patch_ind = 0; patch_ind < npatches; patch_ind++) {
		...
		for (int x = 0; x <= tess; x++) {
			for (int y = 0; y <= tess; y++) {
				...
				point p;
				for (int i = 0; i < 16; i++) {
					p += point(teapot_verts + 3 * patch[i]) * wx[i % 4] * wy[i / 4];
				}
				if (taller) p[2] *= taller_scale;
				int nv = mesh->vertices.size();
				...
				if (patch_grid[x + y * (tess + 1)] == nv) mesh->vertices.push_back(p);
			}
		}
		for (int x = 0; x < tess; x++) {
			for (int y = 0; y < tess; y++) {
