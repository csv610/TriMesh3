...
struct PtPair {
	vec p1, n1, p2, n2;
	PtPair(const point& p1_, const vec& n1_, const point& p2_, const vec& n2_)
...
class NormCompat : public KDtree::CompatFunc {
    ...
    public:
	NormCompat(const vec& n_, TriMesh* m_, float thresh_) : m(m_), n(n_), thresh(thresh_) {}
	virtual bool operator()(const float* p) const {
		int idx = (const point*) p - &(m->vertices[0]);
		return dot(n, m->normals[idx]) > thresh;
	}
};

// Determine which points on mesh1 and mesh2 overlap the other,
// filling in o1 and o2
void compute_overlaps(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, const KDtree* kd1,
		      const KDtree* kd2, vector<float>& o1, vector<float>& o2, float maxdist, int verbose) {
	timestamp t = now();
...
#pragma omp for nowait
		for (int i = 0; i < nv1; i++) {
			...
			if (kd2->exists_pt_within(p, maxdist)) o1[i] = 1;
		}

#pragma omp for
		for (int i = 0; i < nv2; i++) {
			point p = xf21 * mesh2->vertices[i];
			if (kd1->exists_pt_within(p, maxdist)) o2[i] = 1;
		}
	}  // omp parallel
...
static void select_and_match(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, const KDtree* kd2,
			     const vector<float>& sampcdf1, float cdfincr, float maxdist, float angle_thresh,
			     bool do_flip, vector<PtPair>& pairs) {
	xform nxf1 = norm_xf(xf1);
	xform nxf2 = norm_xf(xf2);
	xform xf12 = inv(xf2) * xf1;
	xform nxf12 = norm_xf(xf12);
	bool is_pointcloud1 = (mesh1->faces.empty() && mesh1->tstrips.empty() && mesh1->grid.empty());
	bool is_pointcloud2 = (mesh2->faces.empty() && mesh2->tstrips.empty() && mesh2->grid.empty());
	...
	while (cdfval < 1.0f) {
		if (sampcdf1[i] <= cdfval) {
			...
			step >>= 1;
			while (step) {
				if (i + step < n && sampcdf1[i + step] <= cdfval) i = i + step;
				step >>= 1;
			}
		...
		if (REJECT_BDY && !is_pointcloud && mesh2->is_bdy(match_ind)) continue;
...
static void align_symm(const vector<PtPair>& pairs, float scale, const point& centroid1, const point& centroid2,
	...
	for (size_t i = 0; i < npairs; i++) {
		...
		for (int j = 0; j < 6; j++) {
			b[j] += wt * dn * x[j];
			for (int k = j; k < 6; k++) A[j][k] += wt * x[j] * x[k];
		}
	}
	...
	for (int i = 0; i < 6; i++) einv[i] = 1.0f / (eval[i] + REGULARIZATION * eval[5]);
	...
	alignxf = xform::trans(centroid1) * R * xform::trans(trans) * R * xform::trans(-centroid2);
}

// Do symmetric point-to-plane translation-only alignment
static void align_pt2pl_trans(const vector<PtPair>& pairs, const point& centroid1, const point& centroid2,
			      xform& alignxf) {
	size_t npairs = pairs.size();
	...
	for (size_t i = 0; i < npairs; i++) {
		...
		for (int j = 0; j < 3; j++) {
			b[j] += d * n[j];
			for (int k = 0; k < 3; k++) evec[j][k] += n[j] * n[k];
		}
	}
	...
	for (int i = 0; i < 3; i++) einv[i] = 1.0f / (eval[i] + REGULARIZATION * eval[2]);
...
static void align_scale(const vector<PtPair>& pairs, xform& alignxf, const point& centroid1, const point& centroid2,
			bool do_affine) {
	size_t npairs = pairs.size();
	...
	if (!do_affine) {
		// Just uniform scale
		alignxf = xform::trans(centroid) * xform::scale(sqrt(eval1.sum() / eval2.sum())) *
			  xform::trans(-centroid) * alignxf;
		return;
	}
	...
	alignxf = xform::trans(centroid) * xform::fromarray(csqrt1) * xform::fromarray(icsqrt2) *
		  xform::trans(-centroid) * alignxf;
}
...
static float ICP_iter(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, const KDtree* kd1,
		      const KDtree* kd2, const vector<float>& weights1, const vector<float>& weights2,
		      vector<float>& sampcdf1, vector<float>& sampcdf2, int desired_pairs, float& cdfincr,
		      bool update_cdfs, float& maxdist, float& angle_thresh, int verbose, ICP_xform_type xform_type) {
	const int nv1 = mesh1->vertices.size(), nv2 = mesh2->vertices.size();
	...
	select_and_match(mesh1, mesh2, xf1, xf2, kd2, sampcdf1, cdfincr, maxdist, angle_thresh, false, pairs);
	select_and_match(mesh2, mesh1, xf2, xf1, kd1, sampcdf2, cdfincr, maxdist, angle_thresh, true, pairs);
	...
	for (size_t i = 0; i < npairs; i++) {
		...
		normdots[i] = dot(pairs[i].n1, pairs[i].n2);
	}
	...
	if (median_dist) maxdist = DIST_THRESH_MULT * median_dist;
	float dist2_thresh = sqr(maxdist);
	angle_thresh = clamp(ANGLE_THRESH_MULT * median_angle, ANGLE_THRESH_MIN, ANGLE_THRESH_MAX);
	float normdot_thresh = cos(angle_thresh);

	// Reject
	if (verbose > 1) dprintf("Rejecting pairs with dist > %g or angle > %.1f\n", maxdist, degrees(angle_thresh));
	size_t next = 0;
	for (size_t i = 0; i < npairs; i++) {
		if (distances2[i] > dist2_thresh || normdots[i] < normdot_thresh) continue;
		pairs[next++] = pairs[i];
	}
	...
	npairs = pairs.size();
	if (npairs < MIN_PAIRS) {
		if (verbose) dprintf("Too few point pairs.\n");
		return -1.0f;
	}
	...
	if (!update_cdfs) return err;

	// Construct CDFs based on inverse covariance of normals
	float A1[3][3] = {{0}}, A2[3][3] = {{0}};
	for (int i = 0; i < nv1; i++) {
		if (!weights1[i]) continue;
		for (int j = 0; j < 3; j++)
			for (int k = j; k < 3; k++) A1[j][k] += mesh1->normals[i][j] * mesh1->normals[i][k];
	}
	for (int i = 0; i < nv2; i++) {
		if (!weights2[i]) continue;
		for (int j = 0; j < 3; j++)
			for (int k = j; k < 3; k++) A2[j][k] += mesh2->normals[i][j] * mesh2->normals[i][k];
	}
	...
	for (int i = 0; i < nv1; i++) {
		...
		float s = 0.0f;
		for (int j = 0; j < 3; j++)
			s += eval1[j] * sqr(A1[0][j] * mesh1->normals[i][0] + A1[1][j] * mesh1->normals[i][1] +
	...
	for (int i = 0; i < nv2; i++) {
		...
		float s = 0.0f;
		for (int j = 0; j < 3; j++)
			s += eval2[j] * sqr(A2[0][j] * mesh2->normals[i][0] + A2[1][j] * mesh2->normals[i][1] +
					    A2[2][j] * mesh2->normals[i][2]);
		sum_sampcdf2 += (sampcdf2[i] = s * weights2[i]);
	}

	if (!sum_sampcdf1 || !sum_sampcdf2) {
		if (verbose) dprintf("No overlap.\n");
		return -1.0f;
	}

	float cdf_scale1 = 1 / sum_sampcdf1;
	sampcdf1[0] *= cdf_scale1;
	for (int i = 1; i < nv1 - 1; i++) sampcdf1[i] = cdf_scale1 * sampcdf1[i] + sampcdf1[i - 1];
	sampcdf1[nv1 - 1] = 1.0f;

	float cdf_scale2 = 1 / sum_sampcdf2;
	sampcdf2[0] *= cdf_scale2;
	for (int i = 1; i < nv2 - 1; i++) sampcdf2[i] = cdf_scale2 * sampcdf2[i] + sampcdf2[i - 1];
	sampcdf2[nv2 - 1] = 1.0f;
...
static void make_uniform_cdfs(const vector<float>& weights1, vector<float>& sampcdf1, const vector<float>& weights2,
			      vector<float>& sampcdf2) {
	const int nv1 = weights1.size(), nv2 = weights2.size();
	sampcdf1.resize(nv1);
	sampcdf2.resize(nv2);

	double sum_sampcdf1 = 0, sum_sampcdf2 = 0;
	for (int i = 0; i < nv1; i++) sum_sampcdf1 += (sampcdf1[i] = weights1[i]);
	for (int i = 0; i < nv2; i++) sum_sampcdf2 += (sampcdf2[i] = weights2[i]);
...
float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, const KDtree* kd1, const KDtree* kd2,
	  vector<float>& weights1, vector<float>& weights2, float maxdist /* = 0.0f */, int verbose /* = 0 */,
	...
	if (maxdist <= 0.0f) {
		mesh1->need_bsphere();
		mesh2->need_bsphere();
		maxdist = dist(xf1 * mesh1->bsphere.center, xf2 * mesh2->bsphere.center);
	...
	for (iter = 0; iter < MAX_ITERS; iter++) {
		...
		if (recompute) {
			if (!had_weights) {
				compute_overlaps(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, maxdist,
						 verbose);
			}
		...
		// Do an iteration
		err = ICP_iter(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, sampcdf1, sampcdf2, DESIRED_PAIRS,
			       cdfincr, recompute, maxdist, angle_thresh, verbose, iter_xform_type);
		...
		if (iter - iter_of_min_err >= TERMINATION_ITER_THRESH && iter_of_min_err >= 0 && !recompute &&
	...
	for (iter = 0; iter < FINAL_ITERS; iter++) {
		...
		err = ICP_iter(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, sampcdf1, sampcdf2,
			       DESIRED_PAIRS_FINAL, cdfincr, false, maxdist, angle_thresh, verbose, iter_xform_type);
		if (err < 0) {
			if (!had_weights) {
				weights1.clear();
				weights2.clear();
...
float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, const KDtree* kd1, const KDtree* kd2,
	...
	return ICP(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, 0.0f, verbose, xform_type);
}

float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, int verbose /* = 0 */,
	  ICP_xform_type xform_type /* = ICP_RIGID */) {
	if (verbose > 1) dprintf("\nBuilding KDtrees... ");
	timestamp t = now();
...
#pragma omp parallel for
	...
	if (verbose > 1) dprintf("Done.  %.3f msec.\n", (now() - t) * 1000.0f);

	vector<float> weights1, weights2;
	float icperr = ICP(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, 0.0f, verbose, xform_type);
	delete kd2;
	delete kd1;
	return icperr;
}
...
float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, const KDtree* kd1, const KDtree* kd2,
	  ::std::vector<float>& weights1, ::std::vector<float>& weights2, float maxdist, int verbose, bool do_scale,
	  bool do_affine) {
	ICP_xform_type xform_type = do_affine ? ICP_AFFINE : do_scale ? ICP_SIMILARITY : ICP_RIGID;
