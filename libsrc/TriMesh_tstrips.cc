...
static void tstrip_build(TriMesh& mesh, int f, vector<signed char>& face_avail, vector<int>& todo);
...
void TriMesh::need_tstrips(TstripRep rep) {
	...
	if (faces.empty() && !grid.empty()) need_faces();
	...
	for (int i = 0; i < nf; i++) {
		face_avail[i] = (across_edge[i][0] != -1) + (across_edge[i][1] != -1) + (across_edge[i][2] != -1);
		if (face_avail[i] == 1) todo.push_back(i);
	}
...
static void tstrip_build(TriMesh& mesh, int f, vector<signed char>& face_avail, vector<int>& todo) {
	...
	for (int i = 0; i < 3; i++) {
		...
		if (ae == -1 || face_avail[ae] < 0) continue;
		score[i]++;
		int next_edge = mesh.faces[ae].indexof(v[NEXT_MOD3(i)]);
		int nae = mesh.across_edge[ae][next_edge];
		if (nae == -1 || face_avail[nae] < 0) continue;
		score[i]++;
		if (face_avail[ae] == 2) score[i]++;
	}

	int best_score = max(max(score[0], score[1]), score[2]);
	int best = (score[0] == best_score) ? 0 : (score[1] == best_score) ? 1 : 2;
	...
	while (1) {
		...
		for (int j = 0; j < 3; j++) {
			...
			if (ae == -1) continue;
			if (face_avail[ae] > 0) face_avail[ae]--;
			if (face_avail[ae] == 1) todo.push_back(ae);
		}

		int faceindex = mesh.faces[f].indexof(vlast2);
		if (faceindex < 0) break;
		f = mesh.across_edge[f][faceindex];
		if (f == -1 || face_avail[f] < 0) break;
		vlast2 = vlast1;
		vlast1 = vnext;
...
void TriMesh::unpack_tstrips() {
	std::lock_guard<std::recursive_mutex> lock(computation_mutex);
	if (tstrips.empty() || !faces.empty()) return;
	...
	for (i = 0; i < nstrips; i++) {
		...
		if (flip)
			faces.push_back(Face(tstrips[i - 1], tstrips[i - 2], tstrips[i]));
		else
			faces.push_back(Face(tstrips[i - 2], tstrips[i - 1], tstrips[i]));
		flip = !flip;
		len--;
	}
	dprintf("Done.\n  %d triangles\n", nfaces);
...
void TriMesh::convert_strips(TstripRep rep) {
	if (tstrips.empty()) return;
	if (rep == TSTRIP_TERM && tstrips.back() == -1) return;
