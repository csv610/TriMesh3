...
void TriMesh::need_neighbors() {
	...
	neighbors.resize(nv);
	for (int i = 0; i < nv; i++) neighbors[i].reserve(numneighbors[i]);
	...
	for (int i = 0; i < nf; i++) {
		for (int j = 0; j < 3; j++) {
			...
			if (find(me.begin(), me.end(), n1) == me.end()) me.push_back(n1);
			if (find(me.begin(), me.end(), n2) == me.end()) me.push_back(n2);
		}
	}
...
void TriMesh::need_adjacentfaces() {
	...
	for (int i = 0; i < nv; i++) adjacentfaces[i].reserve(numadjacentfaces[i]);

	for (int i = 0; i < nf; i++) {
		for (int j = 0; j < 3; j++) adjacentfaces[faces[i][j]].push_back(i);
	}
	...
	for (int i = 0; i < nf; i++) {
		for (int j = 0; j < 3; j++) {
			...
			for (size_t k1 = 0; k1 < a1.size(); k1++) {
				...
				if (v2_in_other < 0) continue;
				if (faces[other][NEXT_MOD3(v2_in_other)] != v1) continue;
