...
#pragma omp parallel for
	for (int i = 0; i < nv; i++) mesh->vertices[i] += amount * mesh->normals[i];
	dprintf("Done.\n");
...
#pragma omp parallel for
	for (int i = 0; i < nv; i++) mesh->vertices[i] = xf * mesh->vertices[i];
...
void clip(TriMesh* mesh, const box& b) {
	...
	for (int i = 0; i < nv; i++)
		if (!b.contains(mesh->vertices[i])) toremove[i] = true;
...
point mesh_center_of_mass(TriMesh* mesh) {
	mesh->need_faces();
	if (mesh->faces.empty() && mesh->tstrips.empty()) return point_center_of_mass(mesh->vertices);
...
void point_covariance(const vector<point>& pts, float (&C)[3][3]) {
	for (int j = 0; j < 3; j++)
		for (int k = 0; k < 3; k++) C[j][k] = 0.0f;

	int n = pts.size();
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < 3; j++)
			for (int k = j; k < 3; k++) C[j][k] += pts[i][j] * pts[i][k];
	}

	for (int j = 0; j < 3; j++)
		for (int k = j; k < 3; k++) C[j][k] /= pts.size();
...
void mesh_covariance(TriMesh* mesh, float (&C)[3][3]) {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int v = 0; v < 3; v++) {
			point pc = p[f[v]] - c;
			for (int j = 0; j < 3; j++)
				for (int k = j; k < 3; k++) C[j][k] += vweight * pc[j] * pc[k];
		}

		// Covariance of centroid
		for (int j = 0; j < 3; j++)
			for (int k = j; k < 3; k++) C[j][k] += area * c[j] * c[k];
	}

	for (int j = 0; j < 3; j++)
		for (int k = j; k < 3; k++) C[j][k] /= totarea;
...
void pca_rotate(TriMesh* mesh) {
	...
	int nv = mesh->vertices.size();
	for (int i = 0; i < nv; i++)
		if (dot(mesh->vertices[i], first) > 0.0f) npos++;
	if (npos < nv / 2) first = -first;

	vec second(C[0][1], C[1][1], C[2][1]);
	npos = 0;
	for (int i = 0; i < nv; i++)
		if (dot(mesh->vertices[i], second) > 0.0f) npos++;
	if (npos < nv / 2) second = -second;

	vec third = cross(first, second);
...
static float max_x(const TriMesh* mesh, int i) {
	return max(max(mesh->vertices[mesh->faces[i][0]][0], mesh->vertices[mesh->faces[i][1]][0]),
...
void orient(TriMesh* mesh) {
		...
		while (!q.empty()) {
			...
			for (int j = 0; j < 3; j++) {
				...
				for (size_t k = 0; k < a.size(); k++) {
					int f1 = a[k];
					if (mesh->flags[f1] != NONE) continue;
					int i0 = mesh->faces[f1].indexof(v0);
					int i1 = mesh->faces[f1].indexof(v1);
					if (i0 < 0 || i1 < 0) continue;
					if (i1 == (i0 + 1) % 3) swap(mesh->faces[f1][1], mesh->faces[f1][2]);
					mesh->flags[f1] = cc;
	...
	for (unsigned i = 0; i < cc; i++) {
		...
		if (n[0] < 0.0f) cc_flip[i] = true;
	}

	for (int i = 0; i < nf; i++) {
		if (cc_flip[mesh->flags[i]]) swap(mesh->faces[i][1], mesh->faces[i][2]);
	}
	dprintf("Done.\n");
}
...
void noisify(TriMesh* mesh, float amount) {
	...
	for (int i = 0; i < nv; i++) {
		...
		if (nn) disp[i] /= (float) nn;
		// Normal
		disp[i] += normal_rnd(amount) * mesh->normals[i];
	}
	for (int i = 0; i < nv; i++) mesh->vertices[i] += disp[i];
}
