...
void TriMesh::need_bbox() {
	...
	if (vertices.empty() || bbox.valid()) return;

	dprintf("Computing bounding box... ");

	for (size_t i = 0; i < vertices.size(); i++) bbox += vertices[i];
...
void TriMesh::need_bsphere() {
	std::lock_guard<std::recursive_mutex> lock(computation_mutex);
	if (vertices.empty() || bsphere.valid) return;
...
static int farthest_vertex_along(const TriMesh& t, const vec& dir) {
	...
	int farthest = 0;
	float farthest_dot = dot(v[0], dir);

	for (int i = 1; i < nv; i++) {
		float my_dot = dot(v[i], dir);
		if (my_dot > farthest_dot) farthest = i, farthest_dot = my_dot;
	}
	return farthest;
}
