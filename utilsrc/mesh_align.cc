...
int main(int argc, char* argv[]) {
	...
	while ((c = getopt(argc, argv, "harstvb")) != EOF) {
		switch (c) {
			case 'a':
				xform_type = ICP_AFFINE;
				break;
			case 'r':
				xform_type = ICP_RIGID;
				break;
			case 's':
				xform_type = ICP_SIMILARITY;
				break;
			case 't':
				xform_type = ICP_TRANSLATION;
				break;
			case 'v':
				verbose = 2;
				break;
	...
	auto mesh1 = TriMesh::read(filename1);
	if (!mesh1) usage(argv[0]);
	auto mesh2 = TriMesh::read(filename2);
	...
	timestamp t = now();
	auto kd1 = std::make_unique<KDtree>(mesh1->vertices);
	auto kd2 = std::make_unique<KDtree>(mesh2->vertices);
	if (verbose > 1) {
	...
	if (bulkmode) {
		...
		find_overlap(mesh1.get(), mesh2.get(), xf1, xf2, kd1.get(), kd2.get(), overlap_area, overlap_dist);
	...
	float err = ICP(mesh1.get(), mesh2.get(), xf1, xf2, kd1.get(), kd2.get(), weights1, weights2, 0.0f, verbose,
			xform_type);
	...
	if (bulkmode) {
		...
		if (dot != string::npos) xffilename12.erase(dot);
