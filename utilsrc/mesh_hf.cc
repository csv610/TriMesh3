...
struct Triple {
	int v1, v2, v3;
	float quality;
	Triple(int _v1, int _v2, int _v3, float _q) : v1(_v1), v2(_v2), v3(_v3), quality(_q) {}
	bool operator<(const Triple& rhs) const { return quality < rhs.quality; }
};
...
struct FaceStruct {
	...
	FaceStruct(int _v1, int _v2, int _v3, int _n12 = NO_FACE, int _n23 = NO_FACE, int _n31 = NO_FACE)
	    : v1(_v1), v2(_v2), v3(_v3), n12(_n12), n23(_n23), n31(_n31) {}
};
...
edgeset* find_boundary_edges(const TriMesh* themesh) {
	...
	for (size_t f = 0; f < themesh->faces.size(); f++) {
		for (int i = 0; i < 3; i++) {
			...
			if (!edges->erase(make_pair(v2, v1))) edges->insert(make_pair(v1, v2));
		}
	}
...
void find_initial_edge_neighbors(const TriMesh* themesh, const edgeset* edges,
...
void print_holes(const vector<hole>* holes, bool verbose) {
	...
	for (size_t i = 0; i < holes->size(); i++) holesizes.push_back((*holes)[i].size());
	...
	for (int i = 0; i < nprint; i++) printf("    %d\n", holesizes[i]);
}

// Compute a quality metric for a potential triangle with three vertices
inline float quality(const TriMesh* themesh, float meanedgelen, const vector<VertStruct>& newverts, int v1, int v2,
		     int v3, bool hack = false) {
#define VERT(v)                                                        \
	((size_t(v) < themesh->vertices.size()) ? themesh->vertices[v] \
						: newverts[size_t(v) - themesh->vertices.size()].p)
#define NORM(v)                                                       \
	((size_t(v) < themesh->vertices.size()) ? themesh->normals[v] \
						: newverts[size_t(v) - themesh->vertices.size()].norm)

	if (v1 == v2 || v2 == v3 || v3 == v1) return -1000.0f;
	...
	vec norm = cross(side2, side1);
	normalize(norm);

	float dot1 = dot(norm, NORM(v1));
	float dot2 = dot(norm, NORM(v2));
	float dot3 = dot(norm, NORM(v3));
	if (dot1 < -0.999f || dot2 < -0.999f || dot3 < -0.999f) return -1000.0f;
	...
	if (hack) {
		...
		a = 2.0f + dot(side1, side2);
		// a = sqrt(1.0f/(1.0f - a));
	} else {
		a = sqr(1.0f + dot(side1, side2)) + sqr(1.0f + dot(side2, side3)) + sqr(1.0f + dot(side3, side1));
	}
...
void fill_hole(const TriMesh* themesh, float meanedgelen, const vector<VertStruct>& newverts,
	       map<int, set<int> >& initial_edge_neighbors, const hole& thehole, vector<FaceStruct>& newtris) {
	...
	while (!q.empty()) {
		...
		if (!used[next.v1] && !used[next.v3]) {
			...
			float q13f = quality(themesh, meanedgelen, newverts, thehole[next.v1], thehole[next.v3],
					     thehole[forw], true);
			if (initial_edge_neighbors[thehole[next.v1]].find(thehole[forw]) !=
			    initial_edge_neighbors[thehole[next.v1]].end())
				q13f = -2000.0f;
			float qb13 = quality(themesh, meanedgelen, newverts, thehole[back], thehole[next.v1],
					     thehole[next.v3], true);
...
void find_neighbors(vector<FaceStruct>& tris) {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < n; j++) {
			if (i == j) continue;
			if ((v1 == tris[j].v2 && v2 == tris[j].v1) || (v1 == tris[j].v3 && v2 == tris[j].v2) ||
			    (v1 == tris[j].v1 && v2 == tris[j].v3)) {
				tris[i].n12 = j;
				found++;
				if (found == 3) break;
			} else if ((v2 == tris[j].v2 && v3 == tris[j].v1) || (v2 == tris[j].v3 && v3 == tris[j].v2) ||
				   (v2 == tris[j].v1 && v3 == tris[j].v3)) {
				tris[i].n23 = j;
				found++;
				if (found == 3) break;
			} else if ((v3 == tris[j].v2 && v1 == tris[j].v1) || (v3 == tris[j].v3 && v1 == tris[j].v2) ||
...
bool should_flip(const TriMesh* themesh, float meanedgelen, const vector<VertStruct>& newverts, int v1, int v2, int v3,
		 int v4) {
	// Step 1 - Sanity check
	...
	vec norm123 = trinorm(p1, p2, p3);
	normalize(norm123);
	vec norm134 = trinorm(p1, p3, p4);
	normalize(norm134);
	vec norm124 = trinorm(p1, p2, p4);
	normalize(norm124);
	vec norm234 = trinorm(p2, p3, p4);
	normalize(norm234);
	float old_dot = max(-0.999f, dot(norm123, norm134));
	float new_dot = max(-0.999f, dot(norm124, norm234));
	float old_badness = 1.0f - old_dot / (1.0f + old_dot);
	float new_badness = 1.0f - new_dot / (1.0f + new_dot);
	if (new_badness > 1.5f * old_badness) return false;
	if (new_badness < 0.5f * old_badness) return true;
...
void improve_triangulation(const TriMesh* themesh, float meanedgelen, const vector<VertStruct>& newverts,
			   map<int, set<int> >& initial_edge_neighbors, vector<FaceStruct>& tris) {
	int n = tris.size();
	if (!n) return;
	...
	for (size_t i = 0; i < tris.size(); i++) {
		...
		if (tris[i].v1 < nv) edgeverts.insert(tris[i].v1);
		if (tris[i].v2 < nv) edgeverts.insert(tris[i].v2);
		if (tris[i].v3 < nv) edgeverts.insert(tris[i].v3);
	}
	for (set<int>::const_iterator i = edgeverts.begin(); i != edgeverts.end(); i++) {
		for (set<int>::const_iterator j = initial_edge_neighbors[*i].begin();
		     j != initial_edge_neighbors[*i].end(); j++) {
		...
		if (tris[i].n12 != NO_FACE) {
			...
			if (should_flip(themesh, meanedgelen, newverts, v1, v0, v2, v3)) {
				if (match1 == 1 && edges.find(make_pair(tris[i].v3, tris[j].v2)) == edges.end()) {
					...
					tris[i].n12 = neighbor;
					if (neighbor != NO_FACE) update_neighbor(tris[neighbor], j, i);
					neighbor = tris[i].n23;
					tris[j].n31 = neighbor;
					if (neighbor != NO_FACE) update_neighbor(tris[neighbor], i, j);
					tris[i].n23 = j;
					tris[j].n12 = i;
				} else if (match1 == 2 &&
...
void tesselate_holefill(const TriMesh* themesh, float subdiv_len, vector<VertStruct>& newverts,
	...
	for (int i = 0; i < s; i++) {
		...
		if (dmax < subdiv_len) {
			...
			tris.push_back(FaceStruct(new_ind, tris[i].v2, tris[i].v3, tris[i].n12, tris[i].n23, i));
			tris[i].v2 = new_ind;
			tris[i].n23 = new_tri1;
			int new_tri2 = tris.size();
			tris.push_back(FaceStruct(new_ind, corner, tris[new_tri1].v2, j, neighbor, new_tri1));
			...
			if (tris[j].v1 == tris[new_tri1].v2)
				...
				tris[j].v3 = new_ind;
			if (tris[new_tri1].n23 != NO_FACE) update_neighbor(tris[tris[new_tri1].n23], i, new_tri1);
			if (neighbor != NO_FACE) update_neighbor(tris[neighbor], j, new_tri2);
			...
			newverts.push_back(VertStruct(newpoint, newnorm));
			int new_tri1 = tris.size();
			tris.push_back(FaceStruct(new_ind, tris[i].v3, tris[i].v1, tris[i].n23, tris[i].n31, i));
			tris[i].v3 = new_ind;
			tris[i].n31 = new_tri1;
			int new_tri2 = tris.size();
			tris.push_back(FaceStruct(new_ind, corner, tris[new_tri1].v2, j, neighbor, new_tri1));
			...
			newverts.push_back(VertStruct(newpoint, newnorm));
			int new_tri1 = tris.size();
			tris.push_back(FaceStruct(new_ind, tris[i].v1, tris[i].v2, tris[i].n31, tris[i].n12, i));
			tris[i].v1 = new_ind;
			tris[i].n12 = new_tri1;
			int new_tri2 = tris.size();
			tris.push_back(FaceStruct(new_ind, corner, tris[new_tri1].v2, j, neighbor, new_tri1));
...
void relax_triangulation(TriMesh* themesh, float smooth, float csmooth, int iterations, vector<VertStruct>& newverts,
			 int firstvert, const vector<FaceStruct>& tris) {
	...
	for (int iter = 0; iter < iterations; iter++) {
		...
		for (size_t i = 0; i < tris.size(); i++) {
			...
			if (tris[i].v3 >= firstvert_g) {
				vec othernorm;
				if (tris[i].n12 == NO_FACE) {
					othernorm = NORM(tris[i].v1) + NORM(tris[i].v2);
				} else {
					int j = tris[i].n12;
					othernorm = trinorm(VERT(tris[j].v1), VERT(tris[j].v2), VERT(tris[j].v3));
				}
				normalize(othernorm);
				vec cross_vec = cross(mynorm, othernorm);
				float alpha = -dot(edge12, edge31) / len2(edge12);
				float beta = 1.0f - alpha;
				point p = alpha * v2 + beta * v1;
				float l = csmooth * dist(v3, p);
				if (dot(mynorm, othernorm) > 0.0f) l *= len(cross_vec);
				if (dot(cross_vec, edge12) < 0.0f) l = -l;
				m[tris[i].v3 - firstvert_g] += l * mynorm;
			}
			if (tris[i].v1 >= firstvert_g) {
				vec othernorm;
				if (tris[i].n23 == NO_FACE) {
					othernorm = NORM(tris[i].v2) + NORM(tris[i].v3);
				} else {
					int j = tris[i].n23;
					othernorm = trinorm(VERT(tris[j].v1), VERT(tris[j].v2), VERT(tris[j].v3));
				}
				normalize(othernorm);
				vec cross_vec = cross(mynorm, othernorm);
				float alpha = -dot(edge23, edge12) / len2(edge23);
				float beta = 1.0f - alpha;
				point p = alpha * v3 + beta * v2;
				float l = csmooth * dist(v1, p);
				if (dot(mynorm, othernorm) > 0.0f) l *= len(cross_vec);
				if (dot(cross_vec, edge23) < 0.0f) l = -l;
				m[tris[i].v1 - firstvert_g] += l * mynorm;
			}
			if (tris[i].v2 >= firstvert_g) {
				vec othernorm;
				if (tris[i].n31 == NO_FACE) {
					othernorm = NORM(tris[i].v3) + NORM(tris[i].v1);
				} else {
					int j = tris[i].n31;
					othernorm = trinorm(VERT(tris[j].v1), VERT(tris[j].v2), VERT(tris[j].v3));
				}
				normalize(othernorm);
				vec cross_vec = cross(mynorm, othernorm);
				float alpha = -dot(edge31, edge23) / len2(edge31);
				float beta = 1.0f - alpha;
				point p = alpha * v1 + beta * v3;
				float l = csmooth * dist(v2, p);
				if (dot(mynorm, othernorm) > 0.0f) l *= len(cross_vec);
				if (dot(cross_vec, edge31) < 0.0f) l = -l;
...
int main(int argc, char* argv[]) {
	...
	edgeset* edges = find_boundary_edges(themesh.get());

	map<int, set<int> > initial_edge_neighbors;
	find_initial_edge_neighbors(themesh.get(), edges, initial_edge_neighbors);
	...
	for (size_t i = 0; i < holes->size(); i++) {
		if (maxfill && int((*holes)[i].size()) > maxfill) continue;
		printf("\r                                                                             \r");
		printf("Filling hole %d of %d, size %d: ", (int) i + 1, (int) holes->size(), (int) (*holes)[i].size());
		fflush(stdout);
		int firstnewvert = newverts.size();
		vector<FaceStruct> tmptris;
		printf("f");
		fflush(stdout);
		fill_hole(themesh.get(), mel, newverts, initial_edge_neighbors, (*holes)[i], tmptris);
		if (!fillonly) {
			printf("n");
			fflush(stdout);
			...
			printf("i");
			fflush(stdout);
			improve_triangulation(themesh.get(), mel, newverts, initial_edge_neighbors, tmptris);
			int niters = int(ceil(2.0f * log((float) tmptris.size()) * M_LOG2Ef - 3.0f));
			niters = min(niters, 4);
			for (int j = 0; j < niters; j++) {
				printf("t");
				fflush(stdout);
				tesselate_holefill(themesh.get(), 2.0f * mel, newverts, tmptris);
				printf("i");
				fflush(stdout);
				improve_triangulation(themesh.get(), mel, newverts, initial_edge_neighbors, tmptris);
				printf("r");
				fflush(stdout);
				relax_triangulation(themesh.get(), 0.2f, (nocurv ? 0 : 2), 10, newverts, firstnewvert,
						    tmptris);
				// improve_triangulation(themesh.get(), mel, newverts, initial_edge_neighbors, tmptris);
			}
			...
			printf("t");
			fflush(stdout);
			tesselate_holefill(themesh.get(), 1.5f * mel, newverts, tmptris);
			tesselate_holefill(themesh.get(), mel, newverts, tmptris);
			printf("i");
			fflush(stdout);
			improve_triangulation(themesh.get(), mel, newverts, initial_edge_neighbors, tmptris);
			printf("r");
			fflush(stdout);
			relax_triangulation(themesh.get(), 0.2f, (nocurv ? 0 : 2), 50, newverts, firstnewvert, tmptris);
		}
	...
	if (!themesh->colors.empty()) themesh->colors.resize(nv);
	if (!themesh->confidences.empty()) themesh->confidences.resize(nv);

	if (had_tstrips) themesh->need_tstrips();
