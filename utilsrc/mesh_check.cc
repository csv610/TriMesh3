...
bool bad_vertex(const TriMesh* themesh, int i) {
	...
	if ((x > -MAX_VERTEX_COORD) && (x < MAX_VERTEX_COORD) && (y > -MAX_VERTEX_COORD) && (y < MAX_VERTEX_COORD) &&
...
bool bad_face(const TriMesh* themesh, int i) {
	...
	if ((i0 >= 0) && (i0 < nv) && (i1 >= 0) && (i1 < nv) && (i2 >= 0) && (i2 < nv) && (i0 != i1) && (i1 != i2) &&
	    (i2 != i0))
		return false;
	else
		return true;
...
bool bad_edge(const TriMesh* themesh, int v1, int v2, const vector<bool>& bad_faces) {
...
int main(int argc, char* argv[]) {
	...
	if (num_bad_verts) {
		remove_vertices(in.get(), bad_verts);
		nv = in->vertices.size();
		had_problems = true;
	}
	...
	printf("Looking for bad faces... ");
	fflush(stdout);
	for (int i = 0; i < nf; i++) {
		if (bad_face(in.get(), i)) {
			bad_faces[i] = true;
			num_bad_faces++;
		}
	}
	...
	// ordering.
	printf("Looking for duplicate faces... ");
	fflush(stdout);
	...
	for (int i = 0; i < nf; i++) {
		if (!bad_faces[i]) facelist.push_back(i);
	}
	sort(facelist.begin(), facelist.end(), OrderFace(in.get()));
	...
	for (int i = 0; i < s; i++) {
		...
		while ((i + 1 < s) && (in->faces[facelist[start]][2] == in->faces[facelist[i + 1]][2]) &&
		...
		if (tmp == 0) {
			...
			swap(in->faces[facelist[start]][1], in->faces[facelist[start]][2]);
		}
	}
	...
	if (num_bad_faces || nuked) {
		remove_faces(in.get(), bad_faces);
		nf = in->faces.size();
		had_problems = true;
	}

	// Check for broken topology
	for (int i = 0; i < nf; i++) bad_faces[i] = false;
	num_bad_faces = 0;

	for (int i = 0; i < nv; i++) bad_verts[i] = false;
	num_bad_verts = 0;

	in->need_adjacentfaces();

	printf("Checking for non-manifold topology... ");
	fflush(stdout);
	vector<int> suspect_faces;
	for (int i = 0; i < nf; i++) {
		if (bad_edge(in.get(), in->faces[i][0], in->faces[i][1], bad_faces)) {
		...
		} else if (bad_edge(in.get(), in->faces[i][1], in->faces[i][2], bad_faces)) {
		...
		} else if (bad_edge(in.get(), in->faces[i][2], in->faces[i][0], bad_faces)) {
	...
	if (num_bad_faces) {
		printf("%d one-sided fins... ", num_bad_faces);
		fflush(stdout);
	}
	...
	for (size_t i = 0; i < suspect_faces.size(); i++) {
		...
		if (bad_edge(in.get(), in->faces[f][0], in->faces[f][1], bad_faces) ||
		    bad_edge(in.get(), in->faces[f][1], in->faces[f][2], bad_faces) ||
		    bad_edge(in.get(), in->faces[f][2], in->faces[f][0], bad_faces)) {
	...
	nv = in->vertices.size();
	remove_unused_vertices(in.get());
	int new_nv = in->vertices.size();
	if (new_nv != nv) had_problems = true;
