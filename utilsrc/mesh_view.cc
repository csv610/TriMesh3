...
bool autospin() {
	...
	if (current_mesh >= 0) tmp_xf = global_xf * xforms[current_mesh];

	if (!camera.autospin(tmp_xf)) return false;
...
void initGL() {
	...
	if (gl.slow_tstrips()) avoid_tstrips = true;
}
...
void draw_mesh(int i) {
	TriMesh* mesh = meshes[i].get();
	if (!draw_points && avoid_tstrips) mesh->need_faces();
	...
	bool have_faces = avoid_tstrips ? !mesh->faces.empty() : !mesh->tstrips.empty();
	...
	bool meshcolor = (!draw_index && !draw_falsecolor && have_colors && draw_meshcolor);
	...
	unsigned buf;
	if (!(buf = gl.buffer(mesh->vertices))) buf = gl.make_buffer(mesh->vertices);
	gl.vertexarray3fv(buf);

	if (phong) {
		if (!(buf = gl.buffer(mesh->normals))) buf = gl.make_buffer(mesh->normals);
		gl.normalarray3fv(buf);
	}

	if (meshcolor) {
		if (!(buf = gl.buffer(mesh->colors))) buf = gl.make_buffer(mesh->colors);
	...
	if (faces_pass) {
		if (avoid_tstrips) {
			if (!(buf = gl.ibuffer(mesh->faces))) buf = gl.make_ibuffer(mesh->faces);
			gl.use_ibuffer(buf);
			gl.draw_tris(mesh->faces.size());
		} else {
			if (!(buf = gl.ibuffer(mesh->tstrips))) buf = gl.make_ibuffer(mesh->tstrips);
			gl.use_ibuffer(buf);
	...
	if (edges_pass) {
		...
		if (flat) {
			...
			if (!gl.buffer(mesh->normals)) gl.make_buffer(mesh->normals);
	...
	if (starting_up) {
		...
		while (i < visible.size()) {
			if (!visible[i]) {
				...
				if (turned_on >= 1000000) break;
			}
			i++;
		}
		if (i == visible.size()) starting_up = false;
	}
...
void resetview() {
	...
	// Reload mesh xforms
	for (size_t i = 0; i < meshes.size(); i++)
		if (!xforms[i].read(xfname(filenames[i]))) xforms[i] = xform();

	update_bsph();

	// Set camera to first ".camxf" if we have it...
	for (size_t i = 0; i < filenames.size(); i++) {
		if (global_xf.read(replace_ext(filenames[i], "camxf"))) return;
	}

	// else default view
	global_xf = xform::trans(0, 0, -5.0f * global_bsph.r) * xform::trans(-global_bsph.center);
}
...
void update_title() {
	...
	for (int i = 0; i < nmeshes; i++) {
		if (i == current_mesh || !visible[i]) continue;
		if (strlen(title) + strlen(filenames[i].c_str()) > sizeof(title) - 2) break;
		safe_strcat(title, filenames[i].c_str());
		safe_strcat(title, " ");
	}

	// If nothing visible, just my name
	if (title[0] == '\0') safe_strcat(title, myname);
...
void vis_all() {
	for (size_t i = 0; i < meshes.size(); i++) visible[i] = true;
	update_bsph();
	need_redraw();
	update_title();
}
...
// Hide all meshes
void vis_none() {
	for (size_t i = 0; i < meshes.size(); i++) visible[i] = false;
	current_mesh = -1;
	update_bsph();
	need_redraw();
	update_title();
...
void vis_prev() {
	...
	for (size_t i = 0; i < meshes.size(); i++) visible[i] = (int(i) == prev_vis);
...
void vis_next() {
	...
	int last_vis = -1;
	for (size_t i = 0; i < meshes.size(); i++) {
		if (visible[i]) last_vis = i;
	}
	if (last_vis < 0) last_vis = meshes.size() - 1;

	// Now find the next one
	int next_vis = (last_vis + 1) % meshes.size();

	// Now make only that one visible
	for (size_t i = 0; i < meshes.size(); i++) visible[i] = (int(i) == next_vis);
...
void toggle_vis(int i) {
	if (i >= 0 && i < (int) meshes.size()) visible[i] = !visible[i];
	if (current_mesh == i && !visible[i]) set_current(-1);
	update_bsph();
	need_redraw();
	update_title();
}
...
void dump_image() {
	...
	for (int i = 0; i < height / 2; i++) {
		...
		for (int j = 0; j < 3 * width; j++) swap(row1[j], row2[j]);
	}
...
void do_icp(int n) {
	...
	if (current_mesh < 0 || current_mesh >= (int) meshes.size()) return;
	if (n < 0 || n >= (int) meshes.size()) return;
	if (!visible[n] || !visible[current_mesh] || n == current_mesh) return;
	ICP(meshes[n].get(), meshes[current_mesh].get(), xforms[n], xforms[current_mesh], 2);
	update_bsph();
	need_redraw();
}
...
void auto_icp() {
	...
	if (nmeshes < 2) return;
	if (current_mesh >= 0) {
		for (int icp_target = (current_mesh + nmeshes - 1) % nmeshes; icp_target != current_mesh;
		     icp_target = (icp_target + nmeshes - 1) % nmeshes) {
			if (!visible[icp_target]) continue;
			do_icp(icp_target);
		}
	} else {
		int icp_target, icp_source;
		for (icp_target = 0; icp_target < nmeshes; icp_target++) {
			if (visible[icp_target]) break;
		}
		if (icp_target == nmeshes) return;
		for (icp_source = icp_target + 1; icp_source < nmeshes; icp_source++) {
			if (visible[icp_source]) break;
		}
		if (icp_source == nmeshes) return;
...
void mousehelperfunc(int x, int y) {
	...
	if (current_mesh < 0) {
		camera.mouse(x, y, b, global_xf * global_bsph.center, global_bsph.r, global_xf);
	} else {
		xform tmp_xf = global_xf * xforms[current_mesh];
		camera.mouse(x, y, b, tmp_xf * meshes[current_mesh]->bsphere.center, meshes[current_mesh]->bsphere.r,
			     tmp_xf);
		xforms[current_mesh] = inv(global_xf) * tmp_xf;
		update_bsph();
	}
...
void mousebuttonfunc(int button, int state, int x, int y) {
	...
	if (button == 5 || button == 7) {
		if (state == GLUT_DOWN) vis_prev();
		return;
	} else if (button == 6 || button == 8) {
		if (state == GLUT_DOWN) vis_next();
		return;
	}

	if (state == GLUT_DOWN) {
		buttonstate |= (1 << button);
		if (buttonstate == last_click_buttonstate && now() - last_click_time < doubleclick_threshold &&
		    button < 3) {
...
void keyboardfunc(unsigned char key, int, int) {
	switch (key) {
		case ' ':
			...
			break;
		case '@':  // Shift-2
			draw_2side = !draw_2side;
			break;
		case 'c':
			draw_meshcolor = !draw_meshcolor;
			break;
		case 'e':
			draw_edges = !draw_edges;
			break;
		case 'f':
			draw_falsecolor = !draw_falsecolor;
			break;
		case 'F':
			draw_flat = !draw_flat;
			break;
...
int main(int argc, char* argv[]) {
	...
	if (glutGet(GLUT_INIT_WINDOW_WIDTH) == 1) {
		int window_size = min(glutGet(GLUT_SCREEN_WIDTH), glutGet(GLUT_SCREEN_HEIGHT)) * 3 / 4;
	...
	for (int i = 1; i < argc; i++) {
		...
		auto themesh = TriMesh::read(filename);
		if (!themesh) usage();
		meshes.push_back(std::move(themesh));
		xforms.push_back(xform());
		visible.push_back(false);
		filenames.push_back(filename);
		feature_sizes.push_back(0.0f);	// Computed below
	}
	if (meshes.empty()) usage();
...
#pragma omp parallel for
	for (size_t i = 0; i < meshes.size(); i++) {
		...
		if (meshes[i]->vertices.size() > 240) draw_flat = false;
	}
