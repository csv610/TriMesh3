...
class VHashTable {
    ...
    public:
	VHashTable(size_t maxpoints, float voxelsize_) : scale(1.0f / voxelsize_) {
	...
	data_t& operator[](const point& p) {
		voxel_coord_t c(int(floor(p[0] * scale)), int(floor(p[1] * scale)), int(floor(p[2] * scale)));
...
void crunch(TriMesh* in, TriMesh* out, float voxelsize) {
	...
	for (size_t i = 0; i < onv; i++) out->vertices[i] /= float(out->flags[i]);
	...
	for (size_t i = 0; i < nf; i++) {
		...
		if (ind0 == ind1 || ind0 == ind2 || ind1 == ind2) continue;
...
int main(int argc, char* argv[]) {
	...
	crunch(in.get(), out, voxelsize);

	if (had_tstrips) out->need_tstrips();
	out->write(argv[2]);
}
