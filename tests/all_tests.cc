...
void test_vec_dot_cross_product() {
	...
	ASSERT_LT(dot(parallel, antiparallel), 0.0f);
	ASSERT_FLOAT_EQ(dot(parallel, perp), 0.0f);
}
...
void test_trimesh_bbox() {
	...
	auto cube = std::unique_ptr<TriMesh>(make_cube(1));
	cube->need_bbox();
...
void test_kdtree_closest_point() {
	...
	vector<point> pts = {point(0, 0, 0), point(1, 0, 0), point(2, 0, 0), point(10, 0, 0)};
...
void test_kdtree_ray_query() {
	...
	vector<point> pts = {point(0, 0, 0), point(1, 0, 0), point(2, 0, 0)};
...
void test_kdtree_within_distance() {
	...
	vector<point> pts = {point(0, 0, 0), point(1, 0, 0), point(5, 0, 0)};
...
void test_make_sphere() {
	...
	auto sphere = std::unique_ptr<TriMesh>(make_sphere_polar(16, 16));
...
void test_make_cylinder() {
	...
	auto cyl = std::unique_ptr<TriMesh>(make_cyl(16, 4));
...
void test_make_cone() {
	...
	auto cone = std::unique_ptr<TriMesh>(make_cone(16, 4));
...
void test_make_torus() {
	...
	auto torus = std::unique_ptr<TriMesh>(make_torus(16, 8));
...
void test_make_plane() {
	...
	auto plane = std::unique_ptr<TriMesh>(make_plane(4, 4));
...
void test_make_fixed_shapes() {
	...
	auto tetra = std::unique_ptr<TriMesh>(make_fixed_shape(SHAPE_TETRAHEDRON));
	ASSERT_FALSE(tetra->vertices.empty());

	auto oct = std::unique_ptr<TriMesh>(make_fixed_shape(SHAPE_OCTAHEDRON));
	ASSERT_FALSE(oct->vertices.empty());

	auto icos = std::unique_ptr<TriMesh>(make_fixed_shape(SHAPE_ICOSAHEDRON));
	ASSERT_FALSE(icos->vertices.empty());
}

void test_make_teapot() {
	cout << "    Testing make_teapot..." << endl;
	auto teapot = std::unique_ptr<TriMesh>(make_teapot(4));
...
void test_subdivision_butterfly() {
	...
	subdiv(cube.get(), SUBDIV_BUTTERFLY);
...
void test_noisify() {
	...
	noisify(cube.get(), 0.01f);

	ASSERT_EQ(cube->vertices.size(), (size_t) orig_vert_count);
}
...
void test_bilateral_smooth() {
	...
	auto cube = std::unique_ptr<TriMesh>(make_cube(1));
	bilateral_smooth_mesh(cube.get(), 0.1f, 0.1f);
...
void test_translation() {
	...
	auto mesh = std::unique_ptr<TriMesh>(make_cube(1));
	size_t orig_verts = mesh->vertices.size();

	trans(mesh.get(), vec(1, 0, 0));
...
void test_center_of_mass() {
	...
	auto mesh = std::unique_ptr<TriMesh>(make_cube(1));
	point com = mesh_center_of_mass(mesh.get());
...
void test_connected_components() {
	...
	find_comps(cube.get(), comps, compsizes);
...
void test_select_big_components() {
	...
	auto mesh = std::unique_ptr<TriMesh>(make_cube(1));
	vector<int> comps, compsizes;
	find_comps(mesh.get(), comps, compsizes);
	select_big_comps(mesh.get(), comps, compsizes, 1, 1);
...
void test_curvature_computation() {
	...
	auto mesh = std::unique_ptr<TriMesh>(make_sphere_polar(16, 16));
...
void test_diffuse_curv() {
	...
	auto mesh = std::unique_ptr<TriMesh>(make_sphere_polar(8, 8));
	mesh->need_curvatures();
	diffuse_curv(mesh.get(), 0.1f);
	ASSERT_FALSE(mesh->curv1.empty());
}
...
void test_ply_read_write() {
	...
	auto loaded = TriMesh::read("test_ply.ply");
...
void test_iou_computation() {
	...
	auto mesh1 = std::unique_ptr<TriMesh>(make_cube(1));
	auto mesh2 = std::unique_ptr<TriMesh>(make_cube(1));
	float iou_result = iou(mesh1.get(), mesh2.get());
...
void test_large_mesh() {
	...
	auto sphere = std::unique_ptr<TriMesh>(make_sphere_polar(64, 64));
	sphere->need_normals();
...
int main() {
	...
	auto run_test = [&passed, &failed](const string& name, auto&& func) {
		try {
			func();
			cout << "  [PASS] " << name << "\n";
			passed++;
