...
static inline ::std::string replace_ext(const ::std::string& filename, const ::std::string& ext) {
	using namespace ::std;
	string x = filename;
	string::size_type dot = x.rfind(".", x.length());
	if (dot != string::npos) x.erase(dot);
	if (ext.empty())
		return x;
	else
		return x + string(".") + ext;
...
static inline ::std::string string_printf(const char* format, ...) {
	va_list ap;
	va_start(ap, format);
	int required = ::std::vsnprintf(nullptr, 0, format, ap);
	va_end(ap);
	if (required < 0) return "";
	::std::string s(required, '\0');
	va_start(ap, format);
	::std::vsnprintf(&s[0], required + 1, format, ap);
	va_end(ap);
	return s;
}
...
static inline bool contains(const char* s1, const char* s2) {
	...
	string lower1(len1, '\0');
	for (size_t i = 0; i < len1; i++) lower1[i] = (char) tolower((unsigned char) s1[i]);

	string lower2(len2, '\0');
	for (size_t i = 0; i < len2; i++) lower2[i] = (char) tolower((unsigned char) s2[i]);
