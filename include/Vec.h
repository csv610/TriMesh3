...
Dot product.  There are two ways to write the dot product of v1 and v2:
    f = dot(v1, v2)
    f = v1.dot(v2)
  All are implemented to allow for the range of personal preferences.
...
Functions on vecs:
    ...
    v1 = faceforward(v2,v3,v4); // v2 if (dot(v3, v4) > 0), -v2 otherwise
...
class Vec : public Vec_data<D, T> {
	...
	typedef typename ::std::conditional< ::std::is_integral<T>::value, double, T>::type float_type;
	...
	template <class S>
	inline explicit Vec(S x_, typename ::std::enable_if< ::std::is_arithmetic<S>::value, void>::type* = 0) {
		for (size_type i = 0; i < D; i++) v[i] = x_;
	}

	// Constructor for 1 argument that's a pointer, array, or
	// anything else that can be accessed using [].  Explicit.
	template <class S>
	inline explicit Vec(const S& v_, typename ::std::enable_if<!::std::is_arithmetic<S>::value, void>::type* = 0) {
		for (size_type i = 0; i < D; i++) v[i] = v_[i];
	}
	...
	static inline Vec<D, T> uniform_rnd(T sigma = 1) {
		...
		for (size_type i = 0; i < D; i++) result[i] = ::trimesh::uniform_rnd(sigma);
		return result;
	}
	...
	static inline Vec<D, T> normal_rnd(T sigma = 1) {
		Vec result(VEC_UNINITIALIZED);
		for (size_type i = 0; i < D; i++) result[i] = ::trimesh::normal_rnd(sigma);
		return result;
	}

	// Assignment operator equivalents of the one-parameter constructors
	template <class S>
	inline typename ::std::enable_if< ::std::is_arithmetic<S>::value, Vec&>::type operator=(S x_) {
		for (size_type i = 0; i < D; i++) v[i] = x_;
		return *this;
	}

	template <class S>
	inline typename ::std::enable_if<!::std::is_arithmetic<S>::value, Vec&>::type operator=(const S& v_) {
		for (size_type i = 0; i < D; i++) v[i] = v_[i];
		return *this;
	}
	...
	inline reference operator[](size_type i) { return v[i]; }
	inline reference operator[](int i) { return v[i]; }
	inline const_reference operator[](size_type i) const { return v[i]; }
	inline const_reference operator[](int i) const { return v[i]; }
	...
	inline reference back() { return v[D - 1]; }
	inline const_reference back() const { return v[D - 1]; }

	// Conversion to pointer
	inline operator T*() { return v; }
	inline operator const T*() { return v; }
	inline operator const T*() const { return v; }
	inline pointer data() { return v; }
	inline const_pointer data() const { return v; }

	// Iterators
	inline iterator begin() { return v; }
	inline const_iterator begin() const { return v; }
	inline const_iterator cbegin() const { return v; }
	inline iterator end() { return begin() + D; }
	inline const_iterator end() const { return begin() + D; }
	inline const_iterator cend() const { return begin() + D; }
	...
	// Capacity
	inline size_type size() const { return D; }
	inline size_type max_size() const { return D; }
...
#pragma omp atomic
	...
	inline friend const Vec operator+(const T& x, const Vec& v) {
		using namespace ::std;
		Vec result(VEC_UNINITIALIZED);
		for (size_t i = 0; i < D; i++) result[i] = x + v[i];
		return result;
	}
	inline friend const Vec operator+(const Vec& v, const T& x) {
		using namespace ::std;
		Vec result(VEC_UNINITIALIZED);
		for (size_t i = 0; i < D; i++) result[i] = v[i] + x;
		return result;
	}
	inline friend const Vec operator-(const T& x, const Vec& v) {
		using namespace ::std;
	...
	inline friend void operator==(const Vec&, const T&) { TRIMESH_STATIC_CHECK(!D); }
	inline friend void operator==(const T&, const Vec&) { TRIMESH_STATIC_CHECK(!D); }
	inline friend void operator!=(const Vec&, const T&) { TRIMESH_STATIC_CHECK(!D); }
	inline friend void operator!=(const T&, const Vec&) { TRIMESH_STATIC_CHECK(!D); }
	inline friend void operator>(const Vec&, const T&) { TRIMESH_STATIC_CHECK(!D); }
	inline friend void operator>(const T&, const Vec&) { TRIMESH_STATIC_CHECK(!D); }
...
#pragma omp critical
	...
	inline value_type dot(const Vec& v_) const {
		...
		for (size_type i = 1; i < D; i++) total += v[i] * v_[i];
		return total;
	}
	...
	inline Vec<3, T> cross(const Vec<3, T>& v_) const {
		TRIMESH_STATIC_CHECK(D == 3);
		return Vec<3, T>(v[1] * v_[2] - v[2] * v_[1], v[2] * v_[0] - v[0] * v_[2], v[0] * v_[1] - v[1] * v_[0]);
	}

	// Some partial compatibility with std::valarray, plus generalizations
	inline value_type sum() const {
		value_type total = v[0];
		for (size_type i = 1; i < D; i++) total += v[i];
		return total;
	}
	inline value_type sumabs() const {
		using namespace ::std;
		value_type total = abs(v[0]);
		for (size_type i = 1; i < D; i++) total += abs(v[i]);
		return total;
	}
	inline value_type sumsqr() const {
		value_type total = sqr(v[0]);
		for (size_type i = 1; i < D; i++) total += sqr(v[i]);
		return total;
	}
	inline float_type avg() const { return float_type(sum()) / D; }
	inline float_type avgabs() const { return float_type(sumabs()) / D; }
	inline float_type mean() const { return float_type(sum()) / D; }
	inline float_type meanabs() const { return float_type(sumabs()) / D; }
	...
	inline Vec apply(value_type func(value_type)) const {
		Vec result(VEC_UNINITIALIZED);
		for (size_type i = 0; i < D; i++) result[i] = func(v[i]);
		return result;
	}
...
static inline const Vec<D, T> operator+(const Vec<D, T>& v1, const Vec<D, T>& v2) {
	...
	for (size_t i = 0; i < D; i++) result[i] = v1[i] + v2[i];
	return result;
}
...
static inline const T dot(const Vec<D, T>& v1, const Vec<D, T>& v2) {
	T sum = v1[0] * v2[0];
	for (size_t i = 1; i < D; i++) sum += v1[i] * v2[i];
	return sum;
}
...
static inline ::std::istream& operator>>(::std::istream& is, Vec<D, T>& v) {
	...
	if (!is.good()) v = Vec<D, T>();
	return is;
}

// Vec functions based on GLSL - the scalar ones are in mathutil.h
template <size_t D, class T>
static inline Vec<D, T> faceforward(const Vec<D, T>& N, const Vec<D, T>& I, const Vec<D, T>& Nref) {
...
template <size_t D, class T>
static inline Vec<D, T> refract(const Vec<D, T>& I, const Vec<D, T>& N, const T& eta) {
	using namespace ::std;
	T NdotI = dot(N, I);
...
static inline const T len2(const Vec<D, T>& v) {
	T l2 = sqr(v[0]);
	for (size_t i = 1; i < D; i++) l2 += sqr(v[i]);
	return l2;
}

// Length
template <size_t D, class T>
static inline const typename Vec<D, T>::float_type len(const Vec<D, T>& v) {
	using namespace ::std;
	return sqrt(len2(v));
}
...
template <size_t D, class T>
static inline const typename Vec<D, T>::float_type length(const Vec<D, T>& v) {
	return len(v);
}
...
static inline const T dist2(const Vec<D, T>& v1, const Vec<D, T>& v2) {
	T d2 = sqr(v2[0] - v1[0]);
	for (size_t i = 1; i < D; i++) d2 += sqr(v2[i] - v1[i]);
	return d2;
}

// Distance
template <size_t D, class T>
static inline const typename Vec<D, T>::float_type dist(const Vec<D, T>& v1, const Vec<D, T>& v2) {
	using namespace ::std;
...
template <size_t D, class T>
static inline const typename Vec<D, T>::float_type distance(const Vec<D, T>& v1, const Vec<D, T>& v2) {
	return dist(v1, v2);
}
...
static inline void normalize(Vec<D, T>& v) {
	...
	if (likely(l > 0)) {
		l = 1 / l;
		for (size_t i = 0; i < D; i++) v[i] *= l;
	} else {
		// Make sure we have sane output for length 0 and NaN
		for (size_t i = 0; i < D - 1; i++) v[i] = 0;
		v[D - 1] = 1;
	}
}
...
static inline Vec<3, T> trinorm(const Vec<3, T>& v0, const Vec<3, T>& v1, const Vec<3, T>& v2) {
	return T(0.5) * cross(v1 - v0, v2 - v0);
}

// Angle between two vectors
template <size_t D, class T>
static inline const typename Vec<D, T>::float_type angle(const Vec<D, T>& v1, const Vec<D, T>& v2) {
	using namespace ::std;
...
#define VEC_DECLARE_ONEARG(name)                                             \
	...
	static inline trimesh::Vec<D, T> name(const trimesh::Vec<D, T>& v) { \
		...
		Vec<D, T> result(VEC_UNINITIALIZED);                         \
		for (size_t i = 0; i < D; i++) result[i] = name(v[i]);       \
		return result;                                               \
	}
...
#define VEC_DECLARE_TWOARG_VS(name)                                                      \
	...
	static inline trimesh::Vec<D, T> name(const trimesh::Vec<D, T>& v, const T& a) { \
		...
		using namespace ::trimesh;                                               \
		Vec<D, T> result(VEC_UNINITIALIZED);                                     \
		for (size_t i = 0; i < D; i++) result[i] = name(v[i], a);                \
		return result;                                                           \
	}
#define VEC_DECLARE_TWOARG_SV(name)                                                      \
	...
	static inline trimesh::Vec<D, T> name(const T& a, const trimesh::Vec<D, T>& v) { \
		...
		using namespace ::trimesh;                                               \
		Vec<D, T> result(VEC_UNINITIALIZED);                                     \
		for (size_t i = 0; i < D; i++) result[i] = name(a, v[i]);                \
		return result;                                                           \
	}
#define VEC_DECLARE_TWOARG_VV(name)                                                                       \
	...
	static inline trimesh::Vec<D, T> name(const trimesh::Vec<D, T>& v, const trimesh::Vec<D, T>& w) { \
		...
		using namespace ::trimesh;                                                                \
		Vec<D, T> result(VEC_UNINITIALIZED);                                                      \
		for (size_t i = 0; i < D; i++) result[i] = name(v[i], w[i]);                              \
		return result;                                                                            \
	}
...
#define VEC_DECLARE_THREEARG_VSS(name)                                                               \
	...
	static inline trimesh::Vec<D, T> name(const trimesh::Vec<D, T>& v, const T& a, const T& b) { \
		...
		using namespace ::trimesh;                                                           \
		Vec<D, T> result(VEC_UNINITIALIZED);                                                 \
		for (size_t i = 0; i < D; i++) result[i] = name(v[i], a, b);                         \
		return result;                                                                       \
	}
#define VEC_DECLARE_THREEARG_SSV(name)                                                               \
	...
	static inline trimesh::Vec<D, T> name(const T& a, const T& b, const trimesh::Vec<D, T>& v) { \
		...
		using namespace ::trimesh;                                                           \
		Vec<D, T> result(VEC_UNINITIALIZED);                                                 \
		for (size_t i = 0; i < D; i++) result[i] = name(a, b, v[i]);                         \
		return result;                                                                       \
	}
#define VEC_DECLARE_THREEARG_VVV(name)                                                                  \
	template < ::std::size_t D, class T>                                                            \
	static inline trimesh::Vec<D, T> name(const trimesh::Vec<D, T>& v, const trimesh::Vec<D, T>& w, \
					      const trimesh::Vec<D, T>& x) {                            \
		using namespace ::std;                                                                  \
		using namespace ::trimesh;                                                              \
		Vec<D, T> result(VEC_UNINITIALIZED);                                                    \
		for (size_t i = 0; i < D; i++) result[i] = name(v[i], w[i], x[i]);                      \
		return result;                                                                          \
	}
...
// These are also in namespace trimesh
...
template <size_t D, class T>
static inline void swap(::trimesh::Vec<D, T>& v1, ::trimesh::Vec<D, T>& v2) {
	for (size_t i = 0; i < D; i++) {
		using namespace ::std;
		swap(v1[i], v2[i]);
	}
