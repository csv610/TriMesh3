...
static inline bool ludcmp(T (&A)[N][N], int ind[N]) {
	...
	for (int j = 0; j < N; j++) {
		...
		ind[j] = imax;
		if (unlikely(A[j][j] == 0)) return false;
		if (j != N - 1) {
			T tmp = 1 / A[j][j];
			for (int i = j + 1; i < N; i++) A[i][j] *= tmp;
		}
	}
	return true;
}
...
template <class T, int N>
static inline void lubksb(const T (&A)[N][N], const int ind[N], const T b[N], T x[N]) {
	if (&x[0] != &b[0]) {
...
static inline bool ldltdc(T (&A)[N][N], T rdiag[N]) {
	...
	for (int i = 0; i < N; i++) {
		...
		for (int j = i; j < N; j++) {
			T sum = A[i][j];
			for (int k = 0; k < i; k++) sum -= v[k] * A[j][k];
			if (i == j) {
				if (unlikely(sum == 0)) return false;
				rdiag[i] = 1 / sum;
			} else {
				A[j][i] = sum;
			}
...
template <class T, int N>
static inline void ldltsl(const T (&A)[N][N], const T rdiag[N], const T b[N], T x[N]) {
	for (int i = 0; i < N; i++) {
		T sum = b[i];
...
static inline void eigdc(T (&A)[N][N], T d[N]) {
	...
	for (int i = N - 1; i > 0; i--) {
		T scale = 0;
		for (int k = 0; k < i; k++) scale += abs(d[k]);
		if (scale == 0) {
			e[i] = d[i - 1];
			for (int j = 0; j < i; j++) {
				d[j] = A[i - 1][j];
			...
			for (int j = 0; j < i; j++) {
				f = d[j];
				g = e[j];
				for (int k = j; k < i; k++) A[k][j] -= f * e[k] + g * d[k];
				d[j] = A[i - 1][j];
				A[i][j] = 0;
			}
			d[i] = h;
	...
	for (int i = 0; i < N - 1; i++) {
		...
		if (h != 0) {
			T invh = 1 / h;
			for (int k = 0; k <= i; k++) d[k] = A[k][i + 1] * invh;
			for (int j = 0; j <= i; j++) {
				T g = 0;
	...
	for (int i = 0; i < N - 1; i++) {
		...
		d[k] = d[i];
		d[i] = p;
		for (int j = 0; j < N; j++) swap(A[j][i], A[j][k]);
	}
}

// x <- A * d * A' * b.  x is allowed to be the same as b.
template <class T, int N>
static inline void eigmult(const T (&A)[N][N], const T d[N], const T b[N], T x[N]) {
	T e[N];
	for (int i = 0; i < N; i++) {
		T sum = 0;
	...
	for (int i = 0; i < N; i++) {
		T sum = 0;
		for (int j = 0; j < N; j++) sum += A[i][j] * e[j];
		x[i] = sum;
	}
}
...
static inline void svd(T (&A)[M][N], T s[N], T (&V)[N][N]) {
	...
	const int ncu = std::min(M, N);
	const int nct = std::min(M - 1, N);
	const int nrt = std::max(std::min(M, N - 2), 0);
	const int lu = std::max(nct, nrt);
	...
	for (int k = nct - 1; k >= 0; k--) {
		if (s[k] != 0) {
			...
			for (int i = k; i < M; i++) A[i][k] = -A[i][k];
			A[k][k] += 1;
			for (int i = 0; i < k - 1; i++) A[i][k] = 0;
		} else {
			for (int i = 0; i < M; i++) A[i][k] = 0;
			A[k][k] = 1;
		}
	}
	...
	while (p > 0) {
		...
		if (k != p - 2) {
			int ks = p - 1;
			for (; ks > k; ks--) {
				T t = (ks != p ? abs(e[ks]) : T(0)) + (ks != k + 1 ? abs(e[ks - 1]) : T(0));
		...
		switch (kase) {
			...
			case 3: {
				T scale =
				    1 /
				    std::max(std::max(std::max(std::max(abs(s[p - 1]), abs(s[p - 2])), abs(e[p - 2])),
						      abs(s[k])),
					     abs(e[k]));
				T sp = s[p - 1] * scale;
				T spm1 = s[p - 2] * scale;
