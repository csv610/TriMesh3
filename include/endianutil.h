...
#else
	*(unsigned short*) p = (((*(unsigned short*) p) & 0xff00u) >> 8) | (((*(unsigned short*) p) & 0x00ffu) << 8);
#endif
}
...
#else
	*(unsigned*) p = (((*(unsigned*) p) & 0xff000000u) >> 24) | (((*(unsigned*) p) & 0x00ff0000u) >> 8) |
			 (((*(unsigned*) p) & 0x0000ff00u) << 8) | (((*(unsigned*) p) & 0x000000ffu) << 24);
#endif
}

static inline void swap_64(unsigned char* p) {
#if defined(__GNUC__) && (__GNUC__ * 100 + __GNUC_MINOR__ >= 403)
	*(unsigned long long*) p = __builtin_bswap64(*(unsigned long long*) p);
#elif defined(__linux__)
...
#else
	*(unsigned long long*) p = (((*(unsigned long long*) p) & 0xff00000000000000ull) >> 56) |
