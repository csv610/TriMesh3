...
class Color : public Vec<3, float> {
	...
	inline Color(const Vec<3, double>& v_) : Vec<3, float>((float) v_[0], (float) v_[1], (float) v_[2]) {}
	inline Color(float r_, float g_, float b_) : Vec<3, float>(r_, g_, b_) {}
	inline Color(double r_, double g_, double b_) : Vec<3, float>((float) r_, (float) g_, (float) b_) {}
	explicit inline Color(const float* rgb) : Vec<3, float>(rgb[0], rgb[1], rgb[2]) {}
	explicit inline Color(const double* rgb) : Vec<3, float>((float) rgb[0], (float) rgb[1], (float) rgb[2]) {}
	...
	explicit inline Color(double c) : Vec<3, float>((float) c, (float) c, (float) c) {}
	...
	explicit inline Color(const int* rgb) { *this = Color(rgb[0], rgb[1], rgb[2]); }
	explicit inline Color(const unsigned char* rgb) { *this = Color(rgb[0], rgb[1], rgb[2]); }
	...
	static inline Color orange() { return Color(238, 127, 45); }  // Princeton orange

	// 3x3 color transform - matrix given in *row-major* order
	inline const Color col_transform(float m11, float m12, float m13, float m21, float m22, float m23, float m31,
					 float m32, float m33) const {
		return Color(m11 * v[0] + m12 * v[1] + m13 * v[2], m21 * v[0] + m22 * v[1] + m23 * v[2],
	...
	inline const Color xyz2cielab() const {
		...
		return Color(116.0f * fy - 16.0f, 500.0f * (fx - fy), 200.0f * (fy - fz));
	}
	inline const Color cielab2xyz() const {
		float fy = (v[0] + 16.0f) * (1.0f / 116.0f);
		float fx = fy + v[1] * 0.002f;
		float fz = fy - v[2] * 0.005f;
		return Color(0.95042966f * inv_cielab_nonlinearity(fx), inv_cielab_nonlinearity(fy),
			     1.08880057f * inv_cielab_nonlinearity(fz));
	}

	inline const Color xyz2rgb() const {
		return col_transform(3.24083023f, -1.53731690f, -0.49858927f, -0.96922932f, 1.87593979f, 0.04155444f,
				     0.05564529f, -0.20403272f, 1.05726046f);
	}
	inline const Color rgb2xyz() const {
		return col_transform(0.41240858f, 0.35758962f, 0.18043146f, 0.21264817f, 0.71517924f, 0.07217259f,
	...
	inline const Color rgb2srgb() const {
		return Color(srgb_nonlinearity(v[0]), srgb_nonlinearity(v[1]), srgb_nonlinearity(v[2]));
	}
	inline const Color srgb2rgb() const {
		return Color(inv_srgb_nonlinearity(v[0]), inv_srgb_nonlinearity(v[1]), inv_srgb_nonlinearity(v[2]));
	}
	inline const Color srgb2ycbcr() const {
		return Color(0.0f, 0.5f, 0.5f) + col_transform(0.299f, 0.587f, 0.114f, -0.16873589f, -0.33126411f, 0.5f,
							       0.5f, -0.41868759f, -0.08131241f);
	}
	inline const Color ycbcr2srgb() const {
		return Color(v[0], v[1] - 0.5f, v[2] - 0.5f)
		    .col_transform(1.0f, 0.0f, 1.402f, 1.0f, -0.34413629f, -0.71413629f, 1.0f, 1.772f, 0.0f);
	}
	...
	const Color convert(Colorspace src, Colorspace dst) const {
		...
		if (srcnum < dstnum) switch (src) {
				case CIELAB:
					return (dst == XYZ) ? cielab2xyz() : cielab2xyz().convert(XYZ, dst);
				case XYZ:
					return (dst == RGB) ? xyz2rgb() : xyz2rgb().convert(RGB, dst);
				case RGB:
					return (dst == SRGB) ? rgb2srgb() : rgb2srgb().convert(SRGB, dst);
				default:
					return srgb2ycbcr();
			}
		else
			switch (src) {
				case YCBCR:
					return (dst == SRGB) ? ycbcr2srgb() : ycbcr2srgb().convert(SRGB, dst);
				case SRGB:
					return (dst == RGB) ? srgb2rgb() : srgb2rgb().convert(RGB, dst);
				case RGB:
					return (dst == XYZ) ? rgb2xyz() : rgb2xyz().convert(XYZ, dst);
				default:
					return xyz2cielab();
			}
	}
	...
	inline const Color gamma(Colorspace dst) const {
		switch (dst) {
			case CIELAB:
				return Color(cielab_nonlinearity(v[0]), cielab_nonlinearity(v[1]),
	...
	inline const Color ungamma(Colorspace dst) const {
		switch (dst) {
			case CIELAB:
				return Color(inv_cielab_nonlinearity(v[0]), inv_cielab_nonlinearity(v[1]),
	...
	static inline Color hsv(float H, float S, float V) { return Color(H, S, V).hsv2srgb(); }
};  // class Color
