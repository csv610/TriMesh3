...
class RGBc {
	...
	RGBc(void) {};
	RGBc(unsigned char red, unsigned char green, unsigned char blue) { set(red, green, blue); }
};
#define _RGBC_
#endif
...
class GLUI_String {
	...
	GLUI_String(void) { string[0] = '\0'; }

	GLUI_String(const char* text) { std::strcpy(string, text); }
};
...
class GLUI_Node {
	...
	void dump(FILE* out, const char* name) {
		...
		std::fprintf(out, "   parent: %p     child_head: %p    child_tail: %p\n", parent_node, child_head,
			     child_tail);
		std::fprintf(out, "   next: %p       prev: %p\n", next_sibling, prev_sibling);
	}

	GLUI_Node(void) { parent_node = child_head = child_tail = next_sibling = prev_sibling = nullptr; };
...
class GLUI_Glut_Window : public GLUI_Node {
	...
	GLUI_Glut_Window(void) {
		glut_display_CB = nullptr;
		glut_reshape_CB = nullptr;
		glut_keyboard_CB = nullptr;
		glut_special_CB = nullptr;
		glut_mouse_CB = nullptr;
		glut_motion_CB = nullptr;
		glut_passive_motion_CB = nullptr;
		glut_entry_CB = nullptr;
		glut_visibility_CB = nullptr;
		glut_window_id = 0;
	};
};
...
class GLUI_Control : public GLUI_Node {
	...
	int can_draw(void) { return (glui != nullptr && hidden == false); };
	...
	void translate_to_origin(void) { glTranslatef((float) x_abs + .5f, (float) y_abs + .5f, 0.0f); };
	...
	GLUI_Control(void) {
		...
		int i;
		for (i = 0; i < GLUI_DEF_MAX_ARRAY; i++) float_array_val[i] = last_live_float_array[i] = 0.0;
	};
...
class GLUI : public GLUI_Main {
	...
	GLUI_RadioGroup* add_radiogroup(int* live_var = nullptr, int user_id = -1, GLUI_Update_CB callback = nullptr);

	GLUI_RadioGroup* add_radiogroup_to_panel(GLUI_Panel* panel, int* live_var = nullptr, int user_id = -1,
						 GLUI_Update_CB callback = nullptr);
	GLUI_RadioButton* add_radiobutton_to_group(GLUI_RadioGroup* group, const char* name);

	GLUI_Listbox* add_listbox(const char* name, int* live_var = nullptr, int id = -1,
				  GLUI_Update_CB callback = nullptr);
	GLUI_Listbox* add_listbox_to_panel(GLUI_Panel* panel, const char* name, int* live_var = nullptr, int id = -1,
					   GLUI_Update_CB callback = nullptr);

	GLUI_Rotation* add_rotation(const char* name, float* live_var = nullptr, int id = -1,
				    GLUI_Update_CB callback = nullptr);
	GLUI_Rotation* add_rotation_to_panel(GLUI_Panel* panel, const char* name, float* live_var = nullptr,
					     int id = -1, GLUI_Update_CB callback = nullptr);
	GLUI_Slider* add_slider(const char* name, int data_type = GLUI_SLIDER_INT, float limit_lo = GLUI_SLIDER_LO,
				float limit_hi = GLUI_SLIDER_HI, void* data = nullptr, int id = -1,
				GLUI_Update_CB callback = nullptr);
	GLUI_Slider* add_slider_to_panel(GLUI_Panel* panel, const char* name, int data_type = GLUI_SLIDER_INT,
					 float limit_lo = GLUI_SLIDER_LO, float limit_hi = GLUI_SLIDER_HI,
					 void* data = nullptr, int id = -1, GLUI_Update_CB callback = nullptr);

	GLUI_Translation* add_translation(const char* name, int trans_type, float* live_var = nullptr, int id = -1,
					  GLUI_Update_CB callback = nullptr);
	GLUI_Translation* add_translation_to_panel(GLUI_Panel* panel, const char* name, int trans_type,
						   float* live_var = nullptr, int id = -1,
						   GLUI_Update_CB callback = nullptr);

	GLUI_Checkbox* add_checkbox(const char* name, int* live_var = nullptr, int id = -1,
				    GLUI_Update_CB callback = nullptr);
	GLUI_Checkbox* add_checkbox_to_panel(GLUI_Panel* panel, const char* name, int* live_var = nullptr, int id = -1,
					     GLUI_Update_CB callback = nullptr);

	GLUI_Button* add_button(const char* name, int id = -1, GLUI_Update_CB callback = nullptr);
	GLUI_Button* add_button_to_panel(GLUI_Panel* panel, const char* name, int id = -1,
					 GLUI_Update_CB callback = nullptr);

	GLUI_StaticText* add_statictext(const char* name);
	GLUI_StaticText* add_statictext_to_panel(GLUI_Panel* panel, const char* name);

	GLUI_EditText* add_edittext(const char* name, int data_type = GLUI_EDITTEXT_TEXT, void* live_var = nullptr,
				    int id = -1, GLUI_Update_CB callback = nullptr);
	GLUI_EditText* add_edittext_to_panel(GLUI_Panel* panel, const char* name, int data_type = GLUI_EDITTEXT_TEXT,
					     void* live_var = nullptr, int id = -1, GLUI_Update_CB callback = nullptr);

	GLUI_Spinner* add_spinner(const char* name, int data_type = GLUI_SPINNER_INT, void* live_var = nullptr,
				  int id = -1, GLUI_Update_CB callback = nullptr);
	GLUI_Spinner* add_spinner_to_panel(GLUI_Panel* panel, const char* name, int data_type = GLUI_SPINNER_INT,
					   void* live_var = nullptr, int id = -1, GLUI_Update_CB callback = nullptr);
...
class GLUI_Slider : public GLUI_Control {
	...
	void draw_knob(int x, int off_l, int off_r, int off_t, int off_b, unsigned char r, unsigned char g,
		       unsigned char b);
	...
	virtual void set_int_val(int new_int) {
		...
		int_val = new_int;
		output_live(true);
		if (can_draw()) draw_translated_active_area();
	};
	virtual void set_float_val(float new_float) {
