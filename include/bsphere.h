...
T Basis<D, T>::excess(const Vec<D, T>& p) const {
	T e = -current_sqr_r;
	for (size_t k = 0; k < D; k++) e += sqr(p[k] - current_c[k]);
	return e;
}
...
bool Basis<D, T>::push(const Vec<D, T>& p) {
	...
	if (m == 0) {
		for (size_t i = 0; i < D; i++) c[0][i] = q0[i] = p[i];
		sqr_r[0] = 0;
	} else {
		// set v_m to Q_m
		for (size_t i = 0; i < D; i++) v[m][i] = p[i] - q0[i];

		// compute the a_{m,i}, i < m
		for (size_t i = 1; i < m; i++) {
			a[m][i] = 0;
			for (size_t j = 0; j < D; j++) a[m][i] += v[i][j] * v[m][j];
			a[m][i] *= (T(2) / z[i]);
		}

		// update v_m to Q_m-\bar{Q}_m
		for (size_t i = 1; i < m; i++) {
			for (size_t j = 0; j < D; j++) v[m][j] -= a[m][i] * v[i][j];
		}

		// compute z_m
		z[m] = 0;
		for (size_t j = 0; j < D; j++) z[m] += sqr(v[m][j]);
		z[m] *= T(2);
		...
		// update c, sqr_r
		T e = -sqr_r[m - 1];
		for (size_t i = 0; i < D; i++) e += sqr(p[i] - c[m - 1][i]);
		f[m] = e / z[m];

		for (size_t i = 0; i < D; i++) c[m][i] = c[m - 1][i] + f[m] * v[m][i];
...
T Miniball<D, T>::max_excess(It t, It i, It& pivot) const {
	...
	for (It k = t; k != i; k++) {
		const Vec<D, T>& p = *k;
		e = -sqr_r;
		for (size_t j = 0; j < D; j++) e += sqr(p[j] - c[j]);
		if (e > max_e) {
			max_e = e;
			pivot = k;
		}
