...
static inline T smoothstep(const A& a, const B& b, const T& x) {
	if (unlikely(b <= a)) return step(x, a);
	T t = (x - a) / (b - a);
...
template <class T>
static inline typename ::std::enable_if< ::std::is_integral<T>::value, T>::type uniform_rnd(T n) {
	if (n > 0) {
...
template <class T>
static inline typename ::std::enable_if< ::std::is_floating_point<T>::value, T>::type uniform_rnd(T n) {
	if (n > 0) {
...
template <class T>
static inline typename ::std::enable_if< ::std::is_floating_point<T>::value, T>::type normal_rnd(T sigma) {
	if (sigma == 0) return 0;
...
template <class T>
static inline typename ::std::enable_if< ::std::is_integral<T>::value, double>::type normal_rnd(T sigma) {
