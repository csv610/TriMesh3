...
extern void compute_overlaps(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, const KDtree* kd1,
			     const KDtree* kd2, const Grid* g1, const Grid* g2, ::std::vector<float>& o1,
			     ::std::vector<float>& o2, float& maxdist, int verbose);

// Transformation for which ICP is solving
enum ICP_xform_type { ICP_TRANSLATION, ICP_RIGID, ICP_SIMILARITY, ICP_AFFINE };
...
extern float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, const KDtree* kd1, const KDtree* kd2,
		 ::std::vector<float>& weights1, ::std::vector<float>& weights2, float maxdist = 0.0f, int verbose = 0,
...
extern float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, int verbose = 0,
		 ICP_xform_type xform_type = ICP_RIGID);

// Compatibility interfaces to ICP from before we had ICP_xform_type
extern float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, const KDtree* kd1, const KDtree* kd2,
		 ::std::vector<float>& weights1, ::std::vector<float>& weights2, float maxdist, int verbose,
		 bool do_scale, bool do_affine = false);

extern float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, const KDtree* kd1, const KDtree* kd2,
		 int verbose, bool do_scale, bool do_affine = false);

extern float ICP(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, xform& xf2, int verbose, bool do_scale,
