...
class GLCamera {
    public:
	enum Constraint { UNCONSTRAINED, XCONSTRAINED, YCONSTRAINED, ZCONSTRAINED };
	...
	void mouse_click(int mousex, int mousey, const point& scene_center, float scene_size);
	...
	GLCamera()
	    : lastb(Mouse::NONE),
	      ...
	      dospin(false),
	      spinspeed(0),
	      constraint_(UNCONSTRAINED),
	      field_of_view(0.7f),
	      neardist(0.0f),
	      fardist(0.0f),
	      click_depth(0.0f) {
		lightdir[0] = lightdir[1] = 0;
		lightdir[2] = 1;
		last_time = now();
	}
	...
	void mouse(int mousex, int mousey, Mouse::button b, const point& scene_center, float scene_size, xform& xf);
