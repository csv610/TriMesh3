...
extern void diagonalize_curv(const vec& old_u, const vec& old_v, float ku, float kuv, float kv, const vec& new_norm,
...
// new_u, new_v basis.
extern void proj_curv(const vec& old_u, const vec& old_v, float old_ku, float old_kuv, float old_kv, const vec& new_u,
		      const vec& new_v, float& new_ku, float& new_kuv, float& new_kv);

// Like the above, but for dcurv
extern void proj_dcurv(const vec& old_u, const vec& old_v, const Vec<4> old_dcurv, const vec& new_u, const vec& new_v,
		       Vec<4>& new_dcurv);
...
extern void find_comps(TriMesh* mesh, ::std::vector<int>& comps, ::std::vector<int>& compsizes, bool conn_vert = false);

// Select a particular connected component, and delete all other vertices from
// the mesh.
extern void select_comp(TriMesh* mesh, const ::std::vector<int>& comps, int whichcc);
...
extern void select_big_comps(TriMesh* mesh, const ::std::vector<int>& comps, const ::std::vector<int>& compsizes,
			     int min_size, int total_largest = ::std::numeric_limits<int>::max());
...
extern void select_small_comps(TriMesh* mesh, const ::std::vector<int>& comps, const ::std::vector<int>& compsizes,
			       int max_size, int total_smallest = ::std::numeric_limits<int>::max());

// Find overlap area and RMS distance between mesh1 and mesh2.
// rmsdist is unchanged if area returned as zero
extern void find_overlap(TriMesh* mesh1, TriMesh* mesh2, float& area, float& rmsdist);

extern void find_overlap(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, float& area,
			 float& rmsdist);

class KDtree;
extern void find_overlap(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, const KDtree* kd1,
...
extern float iou(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2);

extern float iou(TriMesh* mesh1, TriMesh* mesh2, const xform& xf1, const xform& xf2, const KDtree* kd1,
		 const KDtree* kd2);
...
enum FixedShape {
	...
	SHAPE_ICOSAHEDRON,
	// Archimedean solids
	SHAPE_TRUNCATED_TETRAHEDRON,
	SHAPE_CUBOCTAHEDRON,
	SHAPE_TRUNCATED_CUBE,
	SHAPE_TRUNCATED_OCTAHEDRON,
	SHAPE_RHOMBICUBOCTAHEDRON,
	SHAPE_TRUNCATED_CUBOCTAHEDRON,
	SHAPE_ICOSIDODECAHEDRON,
	SHAPE_TRUNCATED_DODECAHEDRON,
	SHAPE_TRUNCATED_ICOSAHEDRON,
	SHAPE_SNUB_CUBE,
	SHAPE_RHOMBICOSIDODECAHEDRON,
	SHAPE_TRUNCATED_ICOSIDODECAHEDRON,
	SHAPE_SNUB_DODECAHEDRON,
	// Archimedean duals (Catalan solids)
	SHAPE_TRIAKIS_TETRAHEDRON,
	SHAPE_RHOMBIC_DODECAHEDRON,
	SHAPE_TRIAKIS_OCTAHEDRON,
	SHAPE_TETRAKIS_HEXAHEDRON,
	SHAPE_DELTOIDAL_ICOSITETRAHEDRON,
	SHAPE_DISDYAKIS_DODECAHEDRON,
	SHAPE_RHOMBIC_TRIACONTAHEDRON,
	SHAPE_TRIAKIS_ICOSAHEDRON,
	SHAPE_PENTAKIS_DODECAHEDRON,
	SHAPE_PENTAGONAL_ICOSITETRAHEDRON,
	SHAPE_DELTOIDAL_HEXECONTAHEDRON,
	SHAPE_DISDYAKIS_TRIACONTAHEDRON,
...
// around the z axis.
extern TriMesh* make_surface_of_revolution(int tess_th, const ::std::vector<point>& curve_pts);
